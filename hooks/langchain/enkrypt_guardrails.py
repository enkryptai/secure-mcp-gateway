...
This module provides the core Enkrypt API integration for LangChain.
It enables guardrails protection via BaseCallbackHandler hooks.
...
LOG_DIR = Path(os.environ.get("LANGCHAIN_GUARDRAILS_LOG_DIR", Path.home() / "langchain" / "guardrails_logs"))
LOG_DIR.mkdir(parents=True, exist_ok=True)

LOG_RETENTION_DAYS = int(os.environ.get("LANGCHAIN_GUARDRAILS_LOG_RETENTION_DAYS", "7"))
...
def validate_config(config: dict) -> List[str]:
    ...
    # Validate hook policies (LangChain callback hook names)
    valid_hooks = [
...
# Hook-specific policies (LangChain BaseCallbackHandler hooks)
HOOK_POLICIES = {
    # LLM hooks
    "on_llm_start": CONFIG.get("on_llm_start", {}),
    "on_llm_end": CONFIG.get("on_llm_end", {}),
    "on_llm_error": CONFIG.get("on_llm_error", {}),
    "on_chat_model_start": CONFIG.get("on_chat_model_start", {}),
    # Chain hooks
    "on_chain_start": CONFIG.get("on_chain_start", {}),
    "on_chain_end": CONFIG.get("on_chain_end", {}),
    "on_chain_error": CONFIG.get("on_chain_error", {}),
    # Tool hooks
    "on_tool_start": CONFIG.get("on_tool_start", {}),
    "on_tool_end": CONFIG.get("on_tool_end", {}),
    "on_tool_error": CONFIG.get("on_tool_error", {}),
    # Agent hooks
    "on_agent_action": CONFIG.get("on_agent_action", {}),
    "on_agent_finish": CONFIG.get("on_agent_finish", {}),
    # Retriever hooks
    "on_retriever_start": CONFIG.get("on_retriever_start", {}),
    "on_retriever_end": CONFIG.get("on_retriever_end", {}),
    "on_retriever_error": CONFIG.get("on_retriever_error", {}),
    # Other
...
def check_with_enkrypt_api(text: str, hook_name: str = "on_llm_start") -> tuple:
    """
    ...
    try:
        ...
        resp = session.post(
            ...
            headers={
                ...
                "X-Enkrypt-Source-Name": "langchain-guardrails",
...
def format_violation_message(violations: list, hook_name: str = "on_llm_start") -> str:
