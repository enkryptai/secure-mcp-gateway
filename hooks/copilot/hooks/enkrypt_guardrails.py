...
- userPromptSubmitted (audit-only, output ignored by Copilot)
- preToolUse (blocking — the only hook that can block)
- postToolUse (audit-only, output ignored by Copilot)
- sessionStart (observational, output ignored)
- sessionEnd (observational, output ignored)
- errorOccurred (observational, output ignored)
...
LOG_DIR = Path(os.environ.get("COPILOT_HOOKS_LOG_DIR", Path.home() / "copilot" / "hooks_logs"))
LOG_DIR.mkdir(parents=True, exist_ok=True)

# Log retention settings
LOG_RETENTION_DAYS = int(os.environ.get("COPILOT_HOOKS_LOG_RETENTION_DAYS", "7"))
...
def validate_config(config: dict) -> List[str]:
    ...
    valid_hooks = ["userPromptSubmitted", "preToolUse", "postToolUse", "errorOccurred"]
...
HOOK_POLICIES = {
    "userPromptSubmitted": CONFIG.get("userPromptSubmitted", {}),
    "preToolUse": CONFIG.get("preToolUse", {}),
    "postToolUse": CONFIG.get("postToolUse", {}),
    "errorOccurred": CONFIG.get("errorOccurred", {}),
}
...
def check_with_enkrypt_api(text: str, hook_name: str = "userPromptSubmitted") -> tuple[bool, list, dict]:
    """
    ...
    Args:
        ...
        hook_name: The hook name (userPromptSubmitted, preToolUse, postToolUse, errorOccurred)
    ...
    try:
        ...
        resp = session.post(
            ...
            headers={
                ...
                "X-Enkrypt-Source-Name": "copilot-hooks",
...
def format_violation_message(violations: list, hook_name: str = "userPromptSubmitted") -> str:
...
def check_tool(tool_name: str, tool_args: str) -> tuple[str, str]:
    ...
    # Try to parse tool args and check for sensitive operations
    try:
        params = json.loads(tool_args) if tool_args else {}

        # Check for SQL operations in tool arguments
...
# ============================================================================
# HOOK BASE CLASS (reduces code duplication)
# ============================================================================
...
class BaseHook:
    ...
    def __init__(self, hook_name: str, default_output: dict):
        ...
        Args:
            hook_name: The name of the hook (e.g., "preToolUse")
    ...
    def process(self, data: dict) -> dict:
        ...
        Args:
            data: The parsed JSON input from Copilot

        Returns:
            The JSON output to send back to Copilot
        """
