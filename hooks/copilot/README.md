...
└── hooks/
    ...
    ├── enkrypt_guardrails.py           (moved to src/enkrypt_security/hooks/providers/copilot.py)
...
> `enkrypt-security generate-config --hook <platform>`. The per-platform `guardrails_config.json`
...
```python
from enkrypt_security.hooks.providers.copilot import get_hook_metrics, reset_metrics
