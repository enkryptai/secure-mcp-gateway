...
This module provides the core Enkrypt API integration for Strands Agents hooks.
It enables guardrails protection for ANY model provider in Strands, not just Bedrock.
...
LOG_DIR = Path(os.environ.get("STRANDS_GUARDRAILS_LOG_DIR", Path.home() / "strands" / "guardrails_logs"))
LOG_DIR.mkdir(parents=True, exist_ok=True)

LOG_RETENTION_DAYS = int(os.environ.get("STRANDS_GUARDRAILS_LOG_RETENTION_DAYS", "7"))
...
def validate_config(config: dict) -> List[str]:
    ...
    # Validate hook policies (Strands hook event names)
    valid_hooks = [
...
HOOK_POLICIES = {
    "MessageAdded": CONFIG.get("MessageAdded", {}),
    "BeforeInvocation": CONFIG.get("BeforeInvocation", {}),
    "AfterInvocation": CONFIG.get("AfterInvocation", {}),
    "BeforeModelCall": CONFIG.get("BeforeModelCall", {}),
    "AfterModelCall": CONFIG.get("AfterModelCall", {}),
    "BeforeToolCall": CONFIG.get("BeforeToolCall", {}),
    "AfterToolCall": CONFIG.get("AfterToolCall", {}),
}
...
def get_source_event(hook_name: str) -> str:
    ...
    return event_mapping.get(hook_name, hook_name.lower())


def check_with_enkrypt_api(text: str, hook_name: str = "MessageAdded") -> tuple[bool, list, dict]:
    """
...
def format_violation_message(violations: list, hook_name: str = "MessageAdded") -> str:
