...
This module provides the core Enkrypt API integration for OpenAI Agents SDK hooks.
It enables guardrails protection for agents built with the OpenAI Agents SDK.
...
LOG_DIR = Path(os.environ.get("OPENAI_GUARDRAILS_LOG_DIR", Path.home() / "openai_agents" / "guardrails_logs"))
LOG_DIR.mkdir(parents=True, exist_ok=True)

LOG_RETENTION_DAYS = int(os.environ.get("OPENAI_GUARDRAILS_LOG_RETENTION_DAYS", "7"))
...
def validate_config(config: dict) -> List[str]:
    ...
    # Validate hook policies (OpenAI Agents SDK hook names)
    valid_hooks = [
...
# Hook-specific policies (OpenAI Agents SDK hooks)
HOOK_POLICIES = {
    "on_agent_start": CONFIG.get("on_agent_start", {}),
    "on_agent_end": CONFIG.get("on_agent_end", {}),
    ...
    "on_handoff": CONFIG.get("on_handoff", {}),
}
...
def check_with_enkrypt_api(text: str, hook_name: str = "on_agent_start") -> tuple:
    """
    ...
    try:
        ...
        resp = session.post(
            ...
            headers={
                ...
                "X-Enkrypt-Source-Name": "openai-agents-guardrails",
...
def format_violation_message(violations: list, hook_name: str = "on_agent_start") -> str:
