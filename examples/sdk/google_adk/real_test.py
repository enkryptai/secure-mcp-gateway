"""
=============================================================================
  Google ADK + Real LLM + Enkrypt AI Guardrails Example
=============================================================================

Demonstrates Google Agent Development Kit with real tools:
...
Run:
    ...
    python examples/sdk/google_adk/real_test.py
"""
...
def run_security_tests(guard, method_name):
    ...
    runner = Runner(agent=agent, app_name="enkrypt_demo", session_service=None)
    ...
    try:
        ...
        for event in content:
            if hasattr(event, "content") and event.content:
                print(f"  Response: {str(event.content)[:200]}...")
    ...
    print(f"  Testing dangerous inputs via Runner ({method_name}):")
    blocked_count = 0
    total_tests = 0
    ...
    print(f"\n  Runner-level blocking: {blocked_count}/{total_tests}")
...
# ===================================================================
print_header("PART 1: Google ADK WITHOUT Security")

print("  Creating Google ADK agent with real tools...")
try:
    agent = Agent(
        name="demo_agent",
        model="gemini-2.0-flash",
    ...
    runner = Runner(agent=agent, app_name="enkrypt_demo", session_service=None)
    print("  [OK] Agent and Runner created")
    ...
    try:
        content = runner.run(
            user_id="test_user",
            session_id="test_session",
            new_message="Search for information about Python programming",
        )
        for event in content:
            if hasattr(event, "content") and event.content:
                print(f"  Response: {str(event.content)[:200]}...")
    ...
    print(f"  Error initializing Google ADK: {type(e).__name__}: {e}")
    print("  (Google ADK may require specific Google Cloud setup)")
...
from enkryptai_agent_security.sdk._patch import google_adk as adk_patch
...
adk_patch.install(observer, guard)

print(f"  Setup: Manual GuardrailRegistry + GuardEngine + adk_patch.install()")
