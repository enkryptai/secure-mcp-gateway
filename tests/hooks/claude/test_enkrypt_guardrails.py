#!/usr/bin/env python
"""
Unit tests for Enkrypt AI Guardrails module (Claude Code version).
"""
import json
import os
import sys
...
from enkrypt_security.hooks.providers.claude import (
...
class TestDataclasses(unittest.TestCase):
    ...
    def test_guardrails_config_defaults(self):
        """Test GuardrailsConfig has correct defaults for Claude Code."""
        config = GuardrailsConfig()
        self.assertIsInstance(config.enkrypt_api, EnkryptApiConfig)
        self.assertIsInstance(config.user_prompt_submit, HookPolicy)
...
class TestCheckTool(unittest.TestCase):
    """Test tool checking logic (Claude Code version)."""
...
class TestAnalyzeToolResult(unittest.TestCase):
    """Test tool result analysis (Claude Code version)."""
...
class TestMetricsCollector(unittest.TestCase):
    ...
    def setUp(self):
        ...
        from enkrypt_security.hooks.providers.claude import reset_metrics
        reset_metrics()

    def test_record_call(self):
        """Test recording a hook call."""
        from enkrypt_security.hooks.providers.claude import metrics, get_hook_metrics
    ...
    def test_reset_metrics(self):
        ...
        from enkrypt_security.hooks.providers.claude import metrics, get_hook_metrics, reset_metrics
        metrics.record_call("test_hook", blocked=False, latency_ms=100.0)
...
class TestConfigValidation(unittest.TestCase):
    """Test configuration validation for Claude Code hooks."""

    def test_valid_config(self):
        """Test validation of valid config."""
        from enkrypt_security.hooks.providers.claude import validate_config
        config = {
            "enkrypt_api": {
    ...
    def test_invalid_hook_enabled(self):
        ...
        from enkrypt_security.hooks.providers.claude import validate_config
        config = {"UserPromptSubmit": {"enabled": "yes"}}
        errors = validate_config(config)
    ...
    def test_invalid_block_list(self):
        ...
        from enkrypt_security.hooks.providers.claude import validate_config
        config = {"UserPromptSubmit": {"block": "pii"}}
        errors = validate_config(config)
...
class TestConnectionPooling(unittest.TestCase):
    ...
    def test_session_creation(self):
        ...
        from enkrypt_security.hooks.providers.claude import get_http_session
...
class TestPrecompiledPatterns(unittest.TestCase):
    ...
    def test_patterns_are_compiled(self):
        ...
        import re
        from enkrypt_security.hooks.providers.claude import SENSITIVE_PATTERNS
...
class TestBaseHookClass(unittest.TestCase):
    ...
    def test_input_guardrail_hook_defaults(self):
        """Test InputGuardrailHook default output for Claude Code."""
        from enkrypt_security.hooks.providers.claude import InputGuardrailHook
        ...
        hook = TestHook()
        # Claude Code uses {"decision": "block"} for blocking
    ...
    def test_output_audit_hook_defaults(self):
        ...
        from enkrypt_security.hooks.providers.claude import OutputAuditHook
    ...
    def test_base_hook_is_enabled_property(self):
        ...
        from enkrypt_security.hooks.providers.claude import BaseHook
...
class TestBufferedLogger(unittest.TestCase):
    ...
    def test_buffered_logger_creation(self):
        ...
        from enkrypt_security.hooks.providers.claude import BufferedLogger
...
class TestLogRetention(unittest.TestCase):
    ...
    def test_log_retention_days_default(self):
        ...
        from enkrypt_security.hooks.providers.claude import LOG_RETENTION_DAYS
    ...
    def test_cleanup_old_logs_exists(self):
        ...
        from enkrypt_security.hooks.providers.claude import cleanup_old_logs
...
class TestDynamicReload(unittest.TestCase):
    ...
    def test_reload_config_function_exists(self):
        ...
        from enkrypt_security.hooks.providers.claude import reload_config
        self.assertTrue(callable(reload_config))

    def test_flush_logs_function_exists(self):
        """Test flush_logs function exists."""
        from enkrypt_security.hooks.providers.claude import flush_logs
        self.assertTrue(callable(flush_logs))


class TestClaudeCodeSpecificFeatures(unittest.TestCase):
    """Test Claude Code specific features."""

    def test_hook_policy_mapping(self):
        """Test Claude Code hook names are properly mapped."""
        from enkrypt_security.hooks.providers.claude import HOOK_POLICIES
        # Verify Claude Code hook names exist
        self.assertIn("UserPromptSubmit", HOOK_POLICIES)
        self.assertIn("PreToolUse", HOOK_POLICIES)
        self.assertIn("PostToolUse", HOOK_POLICIES)
        self.assertIn("Stop", HOOK_POLICIES)

    def test_log_dir_uses_claude_path(self):
        """Test LOG_DIR uses claude-specific path."""
        from enkrypt_security.hooks.providers.claude import LOG_DIR
        # Should contain 'claude' not 'cursor'
        log_path_str = str(LOG_DIR)
        self.assertIn("claude", log_path_str.lower())

    def test_check_tool_function_exists(self):
        """Test check_tool function exists (Claude Code version)."""
        from enkrypt_security.hooks.providers.claude import check_tool
        self.assertTrue(callable(check_tool))

    def test_analyze_tool_result_function_exists(self):
        """Test analyze_tool_result function exists (Claude Code version)."""
        from enkrypt_security.hooks.providers.claude import analyze_tool_result
