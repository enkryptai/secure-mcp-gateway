...
log = logging.getLogger("enkryptai_agent_security.sdk.patch.langgraph")
...
def install(
    ...
    global _installed, _orig_invoke, _orig_ainvoke, AGENT_ID
    if _installed:
        return
    if agent_id:
        AGENT_ID = agent_id
...
def _ensure_callback(
    ...
    if isinstance(config, dict):
        ...
        if not any(isinstance(cb, EnkryptLangGraphHandler) for cb in callbacks):
            callbacks = [*callbacks, EnkryptLangGraphHandler(observer, guard_engine, agent_id=AGENT_ID)]
