...
log = logging.getLogger("enkryptai_agent_security.sdk.patch")
...
def sync_checkpoint(guard: GuardEngine | None, checkpoint: str, data: Any, name: str) -> None:
    ...
    if guard.guard_for_checkpoint(checkpoint) is None:
        return
    ...
    if is_input:
        coro = guard.check_input(text, tool_name=name, checkpoint=checkpoint)
    else:
        coro = guard.check_output(text, "", tool_name=name, checkpoint=checkpoint)
...
async def async_checkpoint(guard: GuardEngine | None, checkpoint: str, data: Any, name: str) -> None:
    ...
    if is_input:
        verdict = await guard.check_input(text, tool_name=name, checkpoint=checkpoint)
    else:
        verdict = await guard.check_output(text, "", tool_name=name, checkpoint=checkpoint)
