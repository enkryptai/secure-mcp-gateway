...
log = logging.getLogger("enkryptai_agent_security.sdk.patch.openai_agents")
...
def install(
    ...
    global _installed, _orig_run_descriptor, _orig_run_bound, AGENT_ID
    if _installed:
        return
    if agent_id:
        AGENT_ID = agent_id
    ...
    async def _patched_run(cls: Any, *args: Any, **kwargs: Any) -> Any:
        if "hooks" not in kwargs or kwargs["hooks"] is None:
            kwargs["hooks"] = EnkryptRunHooks(observer, guard_engine, agent_id=AGENT_ID)
