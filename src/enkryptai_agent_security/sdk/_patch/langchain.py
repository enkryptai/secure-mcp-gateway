...
log = logging.getLogger("enkryptai_agent_security.sdk.patch.langchain")
...
def install(
    ...
    global _installed, _custom_agent_id
    if _installed:
        return
    _custom_agent_id = agent_id
...
def _ensure_callback(
    ...
    if isinstance(config, dict):
        ...
        if not any(isinstance(cb, EnkryptLangChainHandler) for cb in callbacks):
            agent_id = _custom_agent_id or getattr(runnable, "name", None) or type(runnable).__name__
