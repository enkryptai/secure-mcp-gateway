...
class _ToolCallContext:
    ...
    async def _check_output(self, output_text: str) -> GuardrailVerdict | None:
        if self._guard is None or self._guard.guard_for_checkpoint("post_tool") is None:
            return None
...
class _StepContext:
    ...
    async def atool_call(
        ...
        if self._guard and self._guard.guard_for_checkpoint("pre_tool") is not None and input_str:
        ...
        try:
            ...
            if ctx._ok and output_text and self._guard and self._guard.guard_for_checkpoint("post_tool") is not None:
                try:
...
class GenericAgentAdapter:
    ...
    def __init__(
        ...
        *,
        ...
        if observer is None:
            from enkryptai_agent_security.sdk._state import get_observer
