...
from enkryptai_agent_security.sdk.encoding import decode_if_encoded
from enkryptai_agent_security.sdk.events import AgentEvent, EventName, GuardrailAction, GuardrailVerdict
from enkryptai_agent_security.sdk.exceptions import (
    GuardrailAPIError,
    GuardrailBlockedError,
    GuardrailTimeoutError,
)
from enkryptai_agent_security.sdk.guardrails.base import (
    GuardrailFactory,
    GuardrailRegistry,
    GuardrailRequest,
    GuardrailResponse,
...
class GuardEngine:
    ...
    def __init__(
        ...
        *,
        pre_llm_policy: dict[str, Any] | None = None,
        pre_tool_policy: dict[str, Any] | None = None,
        post_tool_policy: dict[str, Any] | None = None,
        post_llm_policy: dict[str, Any] | None = None,
        ...
        if registry is None:
            from enkryptai_agent_security.sdk.guardrails.base import GuardrailRegistry as _Reg
            ...
            if guardrail_policy or block:
                ...
                pre_llm_policy = pre_llm_policy or _default
                pre_tool_policy = pre_tool_policy or _default
                post_tool_policy = post_tool_policy or _default
                post_llm_policy = post_llm_policy or _default
        ...
        # Per-checkpoint guards — each checkpoint has its own policy
        self._guards: dict[str, InputGuardrail | OutputGuardrail | None] = {
            "pre_llm": self._factory.create_input_guardrail(pre_llm_policy or {}),
            "pre_tool": self._factory.create_input_guardrail(pre_tool_policy or {}),
            "post_tool": self._factory.create_output_guardrail(post_tool_policy or {}),
            "post_llm": self._factory.create_output_guardrail(post_llm_policy or {}),
        }

        self._pii_handler: PIIHandler | None = self._factory.create_pii_handler(pre_llm_policy or {})

    def guard_for_checkpoint(self, checkpoint: str) -> InputGuardrail | OutputGuardrail | None:
        """Return the guardrail instance for the given checkpoint, or None."""
        return self._guards.get(checkpoint)
    ...
    async def check_input(
        ...
        *,
        ...
        If *checkpoint* is given, the guard for that specific checkpoint is
        used; otherwise falls back to the default input guard (pre_llm).
        """
        import logging
        log = logging.getLogger("enkryptai_agent_security.sdk.guard")

        guard = self._guards.get(checkpoint or "pre_llm")
        if guard is None:
        ...
        log.debug("[GuardEngine] check_input(checkpoint=%s, tool=%s, content=%s...)",
                  checkpoint, tool_name, check_content[:80])
    ...
    async def check_output(
        ...
        *,
        ...
        guard = self._guards.get(checkpoint or "post_llm")
        if guard is None:
            return GuardrailVerdict()
        ...
        try:
            response = await asyncio.wait_for(
                guard.validate(check_content, request),
                timeout=self._timeout,
            )
        except asyncio.TimeoutError:
            if self._fail_open:
