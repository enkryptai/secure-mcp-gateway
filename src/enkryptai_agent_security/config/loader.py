...
logger = logging.getLogger("enkryptai_agent_security.config")
...
def _telemetry_from_dict(data: dict[str, Any]) -> TelemetryConfig:
    ...
    return TelemetryConfig(
        ...
        service_name=str(data.get("service_name", "enkryptai-agent-security")),
...
def _config_from_dict(data: dict[str, Any]) -> EnkryptConfig:
    ...
    # Migration: old configs with top-level input/output_guardrails → sdk.guardrails
    old_input = data.get("input_guardrails")
    old_output = data.get("output_guardrails")
    if old_input or old_output:
        sdk = extra.get("sdk")
        if not isinstance(sdk, SDKSection):
            sdk = SDKSection()
        # Only migrate if sdk.guardrails are all None (not already set)
        has_existing = any(
            getattr(sdk.guardrails, cp) is not None
            for cp in ("pre_llm", "pre_tool", "post_tool", "post_llm")
        )
        if not has_existing:
            cp = CheckpointPolicies()
            if isinstance(old_input, dict):
                gp = _guardrail_from_dict(old_input)
                cp.pre_llm = gp
                cp.pre_tool = gp
            if isinstance(old_output, dict):
                gp = _guardrail_from_dict(old_output)
                cp.post_tool = gp
                cp.post_llm = gp
            sdk.guardrails = cp
            extra["sdk"] = sdk
...
def _apply_env_overrides(config: EnkryptConfig) -> EnkryptConfig:
    ...
    # Applied to all 4 SDK checkpoint guardrails
    val = os.environ.get(ENV_GUARDRAIL_NAME) or os.environ.get(ENV_GUARDRAIL_POLICY)
    if val:
        sdk = config.get_sdk()
        for cp_name in ("pre_llm", "pre_tool", "post_tool", "post_llm"):
            policy = getattr(sdk.guardrails, cp_name)
            if policy is None:
                policy = GuardrailPolicy(enabled=True)
                setattr(sdk.guardrails, cp_name, policy)
            policy.guardrail_name = val
