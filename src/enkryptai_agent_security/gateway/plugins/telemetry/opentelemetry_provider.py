"""OpenTelemetry telemetry provider — backed by enkryptai_agent_security.telemetry.
...
fallbacks) lives in ``enkryptai_agent_security.telemetry.setup`` — this file only
...
from enkryptai_agent_security.gateway.plugins.telemetry.base import TelemetryProvider, TelemetryResult
...
class OpenTelemetryProvider(TelemetryProvider):
    ...
    def _get_common_config(self) -> dict[str, Any]:
        ...
        from enkryptai_agent_security.gateway.utils import _synthesize_plugins_from_unified

        ec = load_config()
        plugins = _synthesize_plugins_from_unified(ec)
        return {**DEFAULT_COMMON_CONFIG, "plugins": plugins}
