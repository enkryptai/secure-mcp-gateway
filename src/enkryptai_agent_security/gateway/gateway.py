...
# Error: Can't find enkryptai_agent_security.gateway
# import importlib
# # Force module initialization to resolve pip installation issues
# try:
#     importlib.import_module("enkryptai_agent_security.gateway")
# except ImportError as e:
#     sys.stderr.write(f"Error importing enkryptai_agent_security.gateway: {e}\n")
#     sys.exit(1)
...
from enkryptai_agent_security.gateway.dependencies import __dependencies__
...
print(
    f"Successfully imported enkryptai_agent_security.gateway v{__version__} in gateway module",
    file=sys.stderr,
)
...
from enkryptai_agent_security.gateway.plugins.auth import initialize_auth_system
from enkryptai_agent_security.gateway.plugins.guardrails import (
    get_guardrail_config_manager,
    initialize_guardrail_system,
)
from enkryptai_agent_security.gateway.plugins.guardrails.example_providers import (
    CustomKeywordProvider,
    OpenAIGuardrailProvider,
)
from enkryptai_agent_security.gateway.services.cache.cache_service import (
    ENKRYPT_GATEWAY_CACHE_EXPIRATION,
    ENKRYPT_MCP_USE_EXTERNAL_CACHE,
    ENKRYPT_TOOL_CACHE_EXPIRATION,
    cache_client,
)
from enkryptai_agent_security.gateway.services.discovery import DiscoveryService
...
from enkryptai_agent_security.gateway.services.timeout import initialize_timeout_manager
...
async def enkrypt_authenticate(ctx: Context):
    ...
    from enkryptai_agent_security.gateway.plugins.auth.config_manager import (
...
async def enkrypt_get_cache_status(ctx: Context):
    ...
    """
    from enkryptai_agent_security.gateway.services.cache.cache_status_service import (
        CacheStatusService,
    )
...
async def enkrypt_clear_cache(
    ...
    """
    from enkryptai_agent_security.gateway.services.cache.cache_management_service import (
