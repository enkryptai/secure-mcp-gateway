...
from enkryptai_agent_security.gateway.consts import (
    CONFIG_PATH,
...
def get_logger():
    ...
    if _logger_cache is None:
        try:
            from enkryptai_agent_security.gateway.plugins.telemetry import (
...
#
# Usage guidance:
# - In most modules, prefer: `from enkryptai_agent_security.gateway.utils import logger`
...
_config_cache = {}
_config_gw_mtime = 0
_config_unified_mtime = 0
_config_path_cached = None
_config_lock = threading.RLock()


def _synthesize_plugins_from_unified(ec) -> dict:
    """Build a ``plugins`` dict from unified config so downstream consumers
    see the same shape they expect (``plugins.auth``, ``plugins.guardrails``,
    ``plugins.telemetry``).
    """
    return {
        "auth": {
            ...
            "config": {
                "api_key": ec.api.api_key,
                "base_url": ec.api.base_url,
            },
        },
        "guardrails": {
            "provider": "enkrypt",
        ...
        "telemetry": {
            "provider": "opentelemetry" if ec.telemetry.enabled else "",
            "config": {
                "enabled": ec.telemetry.enabled,
                "url": ec.telemetry.endpoint or "http://localhost:4317",
                "insecure": ec.telemetry.insecure,
            },
        },
    }
...
def get_common_config(print_debug=False):
    ...
    Loads gateway-specific settings (cache, timeouts, etc.) from
    ``enkrypt_mcp_config.json`` and shared settings (api, telemetry) from
    the unified ``enkrypt_config.json`` via ``load_config()``.

    The ``plugins`` key in the returned dict is synthesized from the unified
    config so all downstream consumers see the same dict shape.

    Uses file modification time to detect config changes and reload
    automatically. Thread-safe for concurrent access.
    """
    global _config_cache, _config_gw_mtime, _config_unified_mtime, _config_path_cached

    # NOTE: Using sys_print here will cause a circular import between
    # get_common_config, is_telemetry_enabled, and sys_print functions.
    ...
    with _config_lock:
        # Determine mtimes for both config files
        gw_mtime = 0.0
        if does_file_exist(picked_config_path):
            try:
                gw_mtime = os.path.getmtime(picked_config_path)
            except OSError as e:
                logger.warning(
        ...
        unified_mtime = 0.0
        try:
            from enkryptai_agent_security.config.loader import load_config, _resolve_config_path
            unified_path = _resolve_config_path(None)
            if unified_path and unified_path.is_file():
                unified_mtime = os.path.getmtime(str(unified_path))
        except Exception:
            pass

        # Return cached config if neither file has changed
        if (
            _config_cache
            and _config_path_cached == picked_config_path
            and gw_mtime == _config_gw_mtime
            and unified_mtime == _config_unified_mtime
        ):
            return _config_cache

        # --- Load gateway config file (for common_mcp_gateway_config only) ---
        gw_config = {}
        if does_file_exist(picked_config_path):
            try:
            ...
            logger.info("[utils] No gateway config file found. Loading example config.")
        ...
        if print_debug and gw_config:
            logger.debug(f"[utils] gw_config: {gw_config}")

        common_config = gw_config.get("common_mcp_gateway_config", {})

        # --- Load unified config for api + telemetry (synthesize plugins) ---
        try:
            from enkryptai_agent_security.config.loader import load_config
            ec = load_config()
            plugins = _synthesize_plugins_from_unified(ec)
        except Exception as e:
            logger.debug(f"[utils] Unified config load failed, using defaults: {e}")
            plugins = {}
        ...
        _config_gw_mtime = gw_mtime
        _config_unified_mtime = unified_mtime
...
def build_log_extra(ctx, custom_id=None, server_name=None, error=None, **kwargs):
    ...
    try:
        ...
        if has_ctx_attrs:
            from enkryptai_agent_security.gateway.plugins.auth import get_auth_config_manager
