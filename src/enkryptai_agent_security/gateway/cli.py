...
# BASE_DIR = os.path.dirname(enkryptai_agent_security.gateway.__file__)
BASE_DIR = files("enkryptai_agent_security.gateway")
...
from enkryptai_agent_security.gateway.version import __version__
                ...
                "mcp_config": [
                    {
                        ...
                        "config": {"command": sys.executable, "args": [ECHO_SERVER_PATH]},
                        "oauth_config": {
                            "enabled": False,
                            "is_remote": False,
                            "OAUTH_VERSION": "2.1",
...
def generate_unified_config(
    *,
    ...
    hook_platforms: list[str] | None = None,
) -> dict:
    """Generate the unified ``enkrypt_config.json`` consumed by all products.

    The gateway section is always included.  ``--sdk`` and ``--hook`` add
    ...
    _input_policy = GuardrailPolicy(
        guardrail_name="Sample Airline Guardrail",
        block=list(_DEFAULT_BLOCKS),
    )
    _output_policy = GuardrailPolicy(
        guardrail_name="Sample Airline Guardrail",
        block=list(_OUTPUT_BLOCKS),
        additional_config={
            "relevancy": False,
    ...
    ec = EnkryptConfig(
        api=EnkryptApiConfig(api_key="YOUR_ENKRYPT_API_KEY"),
        telemetry=TelemetryConfig(
            enabled=True,
            exporter=TelemetryConfig.exporter,  # NONE
            endpoint="http://localhost:4317",
            insecure=True,
        ),
    )

    # -- gateway section (always present) --
    legacy = generate_default_config()
    gateway_section = {
        "admin_apikey": legacy["admin_apikey"],
        "log_level": legacy["common_mcp_gateway_config"].get("enkrypt_log_level", "INFO"),
        ...
        "cache": {
            ...
            "db": 0,
            "password": None,
            "tool_expiration_hours": 4,
            "config_expiration_hours": 24,
        },
        "async_guardrails": {"input": False, "output": False},
        "timeout_settings": legacy["common_mcp_gateway_config"].get("timeout_settings", {}),
        "mcp_configs": legacy["mcp_configs"],
        "projects": legacy["projects"],
        "users": legacy["users"],
        "apikeys": legacy["apikeys"],
    }
    ec.extra["gateway"] = gateway_section

    # -- SDK section --
    if include_sdk:
        ec.extra["sdk"] = SDKSection(
            provider_keys=ProviderKeysConfig(),
            guardrails=CheckpointPolicies(
                pre_llm=_input_policy,
                pre_tool=_input_policy,
                post_tool=_output_policy,
                post_llm=_output_policy,
            ),
        )
    ...
    if hook_platforms:
        ...
        for platform in hook_platforms:
            norm = platform.replace("-", "_")
            if norm in PLATFORM_DEFAULTS:
                hooks[norm] = PLATFORM_DEFAULTS[norm]
            else:
                print(f"WARNING: Unknown hook platform '{platform}', skipping.")
        if hooks:
            ec.extra["hooks"] = hooks
...
def add_or_update_cursor_server(config_path, server_name, command, args, env):
    ...
    server_entry = {"command": command, "args": args, "env": env}
...
def configure_telemetry(config_path, enabled=None, url=None, insecure=None, service_name=None):
    ...
    if service_name is not None:
        config["plugins"]["telemetry"]["config"]["service_name"] = service_name
        updated.append(f"service_name={service_name}")

    if not updated:
        print(
            "ERROR: At least one option (--enabled, --url, --insecure, or --service-name) must be provided"
        )
        sys.exit(1)
...
def start_api_server(host="0.0.0.0", port=8001, reload=False):
    ...
    try:
        ...
        from enkryptai_agent_security.gateway.api_server import app
...
def main():
    ...
    gen_config_parser.add_argument(
        ...
        dest="hooks",
        metavar="PLATFORM",
        help="Add hook platform section (repeatable). Platforms: cursor, claude, claude-code, copilot, kiro, langchain, langgraph, openai, strands, crewai",
    )
    gen_config_parser.add_argument(
        "--sdk",
        action="store_true",
        help="Include SDK section (provider keys, checkpoints, frameworks)",
    )
    gen_config_parser.add_argument(
        "--all",
        action="store_true",
        dest="include_all",
        help="Include gateway + SDK + all hook platforms",
    )
    ...
    config_configure_telemetry_parser.add_argument(
        "--service-name",
        help="Service name for OpenTelemetry (appears as service.name in traces/metrics/logs)",
    )
    ...
    # HOOKS COMMANDS
    # =========================================================================
    hooks_parser = subparsers.add_parser("hooks", help="Install and manage hooks for IDE and framework platforms")
    hooks_subparsers = hooks_parser.add_subparsers(dest="hooks_command", help="Hooks commands")

    # hooks install
    hooks_install_parser = hooks_subparsers.add_parser("install", help="Install hooks for a platform")
    hooks_install_parser.add_argument(
        "--platform", action="append", metavar="NAME",
        help="Target platform (repeatable or comma-separated). "
             "Platforms: cursor, claude, claude-code, copilot, kiro, langchain, langgraph, openai, strands, crewai",
    )
    hooks_install_parser.add_argument(
        "--all", action="store_true", dest="install_all",
        help="Install hooks for all IDE platforms (cursor, claude, claude_code, copilot, kiro)",
    )
    hooks_install_parser.add_argument("--api-key", type=str, help="Enkrypt API key (or set ENKRYPT_API_KEY env var)")
    hooks_install_parser.add_argument(
        "--scope", choices=["global", "project"],
        help="Install scope (default: platform-dependent)",
    )
    ...
    hooks_list_parser.add_argument("--project-dir", type=str, help="Project directory (default: current directory)")

    # hooks status
    hooks_status_parser = hooks_subparsers.add_parser("status", help="Show detailed status for a platform")
    hooks_status_parser.add_argument("--platform", type=str, required=True, metavar="NAME", help="Target platform")
    hooks_status_parser.add_argument("--project-dir", type=str, help="Project directory (default: current directory)")

    # hooks uninstall
    hooks_uninstall_parser = hooks_subparsers.add_parser("uninstall", help="Remove hooks for a platform")
    hooks_uninstall_parser.add_argument(
        "--platform", action="append", metavar="NAME",
        help="Target platform (repeatable or comma-separated).",
    )
    hooks_uninstall_parser.add_argument(
        "--all", action="store_true", dest="uninstall_all",
        help="Uninstall hooks for all IDE platforms",
    )
    hooks_uninstall_parser.add_argument(
        "--scope", choices=["global", "project"],
        help="Scope to uninstall from (default: platform-dependent)",
    )
    hooks_uninstall_parser.add_argument("--project-dir", type=str, help="Project directory (default: current directory)")
    hooks_uninstall_parser.add_argument(
        "--keep-guardrails", action="store_true",
        help="Keep guardrails config file (only remove hook config files)",
    )

    # hooks disable
    hooks_disable_parser = hooks_subparsers.add_parser("disable", help="Disable guardrail policies for a platform")
    hooks_disable_parser.add_argument("--platform", type=str, required=True, metavar="NAME", help="Target platform")
    hooks_disable_parser.add_argument(
        "--hook", type=str, metavar="HOOK_NAME",
        help="Disable only a specific hook (e.g., beforeMCPExecution). Default: all hooks.",
    )

    # hooks enable
    hooks_enable_parser = hooks_subparsers.add_parser("enable", help="Enable guardrail policies for a platform")
    hooks_enable_parser.add_argument("--platform", type=str, required=True, metavar="NAME", help="Target platform")
    hooks_enable_parser.add_argument(
        "--hook", type=str, metavar="HOOK_NAME",
        help="Enable only a specific hook (e.g., beforeMCPExecution). Default: all hooks.",
    )
    ...
    if args.command == "hooks" and not getattr(args, "hooks_command", None):
    ...
    # =========================================================================
        ...
        # Determine unified config path
        unified_config_dir = os.path.join(HOME_DIR, ".enkrypt")
        unified_config_path = os.path.join(unified_config_dir, "enkrypt_config.json")
        ...
        print("INFO: Generating default gateway configuration...")
        config = generate_default_config()
        with open(config_path, "w") as f:
            json.dump(config, f, indent=2)
        print(f"SUCCESS: Generated gateway config at {config_path}")

        # --- Unified config ---
        include_all = getattr(args, "include_all", False)
        include_sdk = getattr(args, "sdk", False) or include_all
        hook_platforms = getattr(args, "hooks", None) or []
        if include_all:
            from enkryptai_agent_security.config.hook_defaults import SUPPORTED_PLATFORMS
            hook_platforms = list(SUPPORTED_PLATFORMS)

        # Merge into existing unified config if it exists
        existing_unified: dict = {}
        ...
        unified = generate_unified_config(
            include_sdk=include_sdk,
            hook_platforms=hook_platforms if hook_platforms else None,
        )

        # Merge: new sections are added, existing ones preserved unless overwriting
        if existing_unified and not args.overwrite:
            # Merge hooks: add new platforms without removing existing ones
            if "hooks" in existing_unified and "hooks" in unified:
                merged_hooks = dict(existing_unified["hooks"])
                merged_hooks.update(unified["hooks"])
                unified["hooks"] = merged_hooks
            elif "hooks" in existing_unified:
                unified["hooks"] = existing_unified["hooks"]

            # Preserve existing SDK if not explicitly requested
            if "sdk" in existing_unified and "sdk" not in unified:
                unified["sdk"] = existing_unified["sdk"]
        elif args.overwrite and os.path.exists(unified_config_path):
            backup_fn = f"enkrypt_config.json.bkp.{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            backup_p = os.path.join(unified_config_dir, backup_fn)
            try:
                shutil.copy2(unified_config_path, backup_p)
                print(f"INFO: Unified config backup at {backup_p}")
            except Exception:
                pass

        os.makedirs(unified_config_dir, exist_ok=True)
        with open(unified_config_path, "w", encoding="utf-8") as f:
            json.dump(unified, f, indent=2)
        print(f"SUCCESS: Generated unified config at {unified_config_path}")

        parts = ["gateway"]
        if include_sdk:
            parts.append("sdk")
        if hook_platforms:
            parts.append(f"hooks ({', '.join(hook_platforms)})")
        print(f"INFO: Sections included: {', '.join(parts)}")
        sys.exit(0)
        ...
        if args.client.lower() == "claude" or args.client.lower() == "claude-desktop":
            ...
            if is_docker_running:
                ...
                # non-Docker logic — write config directly (like Cursor)
                # instead of relying on `mcp install` which uses the system mcp
                ...
                if os.path.exists(claude_desktop_config_path):
                    try:
                        ...
                        print(
                            f"ERROR: Error parsing {claude_desktop_config_path}: {e!s}"
                        )
                        sys.exit(1)
        ...
        from enkryptai_agent_security.gateway.hooks_installer import handle_hooks_command
        handle_hooks_command(args)

    else:
        print(
            "INFO: ",
            f"Invalid command: {args.command}. Please use 'generate-config', 'install', 'hooks', 'config', 'project', 'user', or 'system'.",
        )
        parser.print_help()
        sys.exit(1)
