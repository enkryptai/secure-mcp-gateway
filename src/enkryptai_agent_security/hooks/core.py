...
log = logging.getLogger("enkryptai_agent_security.hooks")
...
def _resolve_scope(config_path: Path) -> str:
    """Determine if a config path is global, project-scoped, or env-overridden."""
    try:
    ...
    if env:
        try:
            if str(Path(env).resolve()) == resolved:
                return "env"
        except OSError:
            pass
    if resolved.startswith(os.path.join(home, ".enkrypt") + os.sep) or \
       resolved.startswith(os.path.join(home, ".claude") + os.sep):
        return "global"
    return "project"
...
class HooksCore:
    ...
    def from_config_file(
        ...
        *,
        ...
        config = _load_json_config(resolved)

        # Unified config detection — the file has a top-level "api" key
        if config and "api" in config:
            try:
                from enkryptai_agent_security.config.loader import _config_from_dict
        ...
        if not config:
            try:
                from enkryptai_agent_security.config import load_config as _load_shared
    ...
    def from_enkrypt_config(
        ...
        *,
        ...
        """
        from enkryptai_agent_security.config.models import HookPlatformConfig as _HPC
    ...
    def enable_telemetry(
        ...
        *,
        ...
        try:
            from enkryptai_agent_security.telemetry.setup import init_telemetry
