...
from enkrypt_security.telemetry import SourceProduct, SpanAttributes, SpanNames
from enkrypt_security.gateway.client import forward_tool_call

# Telemetry components will be obtained lazily when needed
from enkrypt_security.gateway.error_handling import create_error_response, error_logger
from enkrypt_security.gateway.exceptions import (
    ErrorCode,
    ErrorContext,
    create_discovery_error,
)
from enkrypt_security.gateway.plugins.auth import get_auth_config_manager
from enkrypt_security.gateway.plugins.telemetry import get_telemetry_config_manager
from enkrypt_security.gateway.services.cache.cache_service import cache_service
...
class DiscoveryService:
    ...
    async def _check_authentication(
        ...
        if not self.auth_manager.is_session_authenticated(session_key):
            with tracer_obj.start_as_current_span(SpanNames.AUTH) as auth_span:
                ...
                from enkrypt_security.gateway.gateway import enkrypt_authenticate
    ...
    async def _discover_all_servers(
        ...
        with tracer_obj.start_as_current_span(SpanNames.DISCOVERY) as all_span:
            ...
            # Import here to avoid circular imports
            from enkrypt_security.gateway.gateway import enkrypt_list_all_servers
    ...
    async def _discover_single_server(
        ...
        # Server info check
        with tracer_obj.start_as_current_span(SpanNames.DISCOVERY) as info_span:
            ...
            if config_tools:
                ...
                try:
                    from enkrypt_security.gateway.client import get_server_metadata_only
        ...
        # Tool discovery
        with tracer_obj.start_as_current_span(SpanNames.DISCOVERY) as discover_span:
            discover_span.set_attribute(SpanAttributes.SERVER_NAME, server_name)

            # Cache check
            with tracer_obj.start_as_current_span(SpanNames.DISCOVERY_CACHE_CHECK) as cache_span:
                cached_tools = self.cache_service.get_cached_tools(id, server_name)
            ...
            # Forward tool call
            with tracer_obj.start_as_current_span(SpanNames.DISCOVERY_FORWARD) as tool_span:
                # Update metrics lazily
                telemetry_manager = get_telemetry_config_manager()
                if (
                ...
                if tools:
                    ...
                    # Cache write
                    with tracer_obj.start_as_current_span(
                        SpanNames.DISCOVERY_CACHE_STORE
                    ) as cache_write_span:
