...
from enkrypt_security.telemetry import SpanAttributes, SpanNames
...
class ServerListingService:
    ...
    async def list_servers(
        ...
        with tracer.start_as_current_span(SpanNames.SERVER_LIST) as main_span:
    ...
    async def _check_authentication(
        ...
        with tracer.start_span(SpanNames.AUTH) as auth_span:
    ...
    async def _process_servers(
        ...
        with tracer.start_span(SpanNames.SERVER_PROCESS) as process_span:
            ...
            if IS_DEBUG_LOG_LEVEL:
                ...
                for server in mcp_config:
                    ...
                    if "config" in masked_server and "env" in masked_server["config"]:
                        ...
                        from enkrypt_security.gateway.utils import mask_sensitive_env_vars
            ...
            for server_info in mcp_config:
                ...
                # Check server cache
                with tracer.start_span(SpanNames.DISCOVERY_CACHE_CHECK) as cache_span:
    ...
    async def _discover_and_return_servers(
        ...
        with tracer.start_span(SpanNames.DISCOVERY) as discover_span:
            ...
            async def _discover_single(server_name: str):
                with tracer.start_span(SpanNames.DISCOVERY) as server_span:
