...
#
# Usage guidance:
# - In most modules, prefer: `from enkrypt_security.gateway.utils import logger`
...
def get_common_config(print_debug=False):
    ...
    with _config_lock:
        ...
        # Try unified config if no gateway config was found
        if not config:
            try:
                from enkrypt_security.config.loader import (
                    _config_from_dict,
                    _load_json_file,
                    to_gateway_config,
                )
                ...
                unified_path = Path.home() / ".enkrypt" / "enkrypt_config.json"
                if unified_path.is_file():
                    logger.info("[utils] Found unified config — converting to gateway format")
                    raw = _load_json_file(unified_path)
                    if raw:
                        ec = _config_from_dict(raw)
                        gw_dict = to_gateway_config(ec)
                        gateway_section = gw_dict.pop("plugins", {})
                        _config_cache = {
                            **DEFAULT_COMMON_CONFIG,
                            **gw_dict,
                            "plugins": gateway_section,
                        }
                        return _config_cache
            except Exception as e:
                logger.debug(f"[utils] Unified config fallback failed: {e}")
