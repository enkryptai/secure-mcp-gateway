...
# Error: Can't find enkrypt_security.gateway
# import importlib
# # Force module initialization to resolve pip installation issues
# try:
#     importlib.import_module("enkrypt_security.gateway")
# except ImportError as e:
#     sys.stderr.write(f"Error importing enkrypt_security.gateway: {e}\n")
#     sys.exit(1)
...
from enkrypt_security.gateway.dependencies import __dependencies__
from enkrypt_security.gateway.plugins.auth import get_auth_config_manager
...
print(
    f"Successfully imported enkrypt_security.gateway v{__version__} in gateway module",
    file=sys.stderr,
)

# Initialize telemetry system with plugin-based architecture
from enkrypt_security.gateway.plugins.telemetry import (
...
from enkrypt_security.gateway.plugins.auth import initialize_auth_system
from enkrypt_security.gateway.plugins.guardrails import (
    get_guardrail_config_manager,
    initialize_guardrail_system,
)
from enkrypt_security.gateway.plugins.guardrails.example_providers import (
    CustomKeywordProvider,
    OpenAIGuardrailProvider,
)
from enkrypt_security.gateway.services.cache.cache_service import (
    ENKRYPT_GATEWAY_CACHE_EXPIRATION,
    ENKRYPT_MCP_USE_EXTERNAL_CACHE,
    ENKRYPT_TOOL_CACHE_EXPIRATION,
    cache_client,
)
from enkrypt_security.gateway.services.discovery import DiscoveryService
from enkrypt_security.gateway.services.server.server_info_service import ServerInfoService
from enkrypt_security.gateway.services.server.server_listing_service import (
...
from enkrypt_security.gateway.services.timeout import initialize_timeout_manager
...
async def enkrypt_authenticate(ctx: Context):
    ...
    from enkrypt_security.gateway.plugins.auth.config_manager import (
...
async def enkrypt_get_cache_status(ctx: Context):
    ...
    """
    from enkrypt_security.gateway.services.cache.cache_status_service import (
        CacheStatusService,
    )
...
async def enkrypt_clear_cache(
    ...
    """
    from enkrypt_security.gateway.services.cache.cache_management_service import (
