"""Enkrypt guardrail provider — backed by ``enkrypt_security.guardrails``.
...
from enkrypt_security.guardrails import (
    EnkryptGuardrailClient,
)
from enkrypt_security.guardrails import (
    GuardrailResult as SharedResult,
)
from enkrypt_security.gateway.error_handling import error_handling_context, error_logger
from enkrypt_security.gateway.exceptions import (
    ErrorCode,
    ErrorContext,
    create_guardrail_error,
)
from enkrypt_security.gateway.plugins.guardrails.base import (
    GuardrailAction,
    GuardrailProvider,
    GuardrailRequest,
    GuardrailResponse,
...
from enkrypt_security.gateway.utils import logger
...
class EnkryptServerRegistrationGuardrail:
    ...
    def _handle_tool_validation_error(self, e: Exception) -> GuardrailResponse:
        ...
        from enkrypt_security.gateway.exceptions import TimeoutError as GWTimeoutError
...
class EnkryptGuardrailProvider(GuardrailProvider):
    ...
    def _get_api_credentials(self) -> tuple[str, str]:
        ...
        picked = DOCKER_CONFIG_PATH if is_docker() else CONFIG_PATH
        if does_file_exist(picked):
        ...
        gr_config = plugins.get("guardrails", {}).get("config", {})
        auth_config = plugins.get("auth", {}).get("config", {})

        api_key = gr_config.get("api_key", auth_config.get("api_key", self.api_key))
        base_url = gr_config.get("base_url", auth_config.get("base_url", self.base_url))
