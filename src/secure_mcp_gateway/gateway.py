...
from secure_mcp_gateway.client import (
    ...
    get_cached_tools,
    get_gateway_config_hashed_key,
    get_id_from_key,
    get_server_hashed_key,
    initialize_cache,
)
...
def get_redis_ttl(cache_client, key):
    """
    ...
    Returns:
        tuple: (ttl_seconds, expires_at_timestamp) or (None, None) if key doesn't exist or has no expiration
    """
    if not cache_client or not key:
        return None, None

    try:
        ttl_seconds = cache_client.ttl(key)
        if ttl_seconds == -1:
        ...
        if IS_DEBUG_LOG_LEVEL:
            sys_print(
                f"[get_redis_ttl] Error getting TTL for key {key}: {e}", is_debug=True
            )
        return None, None


def guardrail_response_has_pii_redaction(guardrail_response):
    """
    ...
    with tracer.start_as_current_span("enkrypt_authenticate") as main_span:
        try:
            ...
            with tracer.start_as_current_span("get_credentials") as cred_span:
                ...
                local_mcp_config = get_local_mcp_config(
                ...
                cred_span.set_attribute(
                    "mcp_config_id", mcp_config_id or "not_provided"
                )
            ...
            with tracer.start_as_current_span("check_session") as session_span:
                ...
                is_authenticated = (
                    session_key in SESSIONS and SESSIONS[session_key]["authenticated"]
                )
                ...
                if is_authenticated:
                    if IS_DEBUG_LOG_LEVEL:
                        sys_print(
                            "[authenticate] Already authenticated in session",
                            is_debug=True,
                        )

                    mcp_config = SESSIONS[session_key]["gateway_config"].get(
                        "mcp_config", []
                    )
            ...
            unique_id = (
                f"{user_id}_{project_id}_{mcp_config_id}"
                if all([user_id, project_id, mcp_config_id])
                else f"{gateway_key}_{int(time.time())}"
            )
            ...
            with tracer.start_as_current_span("lookup_cached_id") as id_span:
                ...
                if cached_id:
                    ...
                    if IS_DEBUG_LOG_LEVEL:
                        sys_print(
                            f"[authenticate] Found cached ID: {cached_id}",
                            is_debug=True,
                        )
            ...
            if cached_id:
                with tracer.start_as_current_span(
                    ...
                    if cached_config:
                        ...
                        if IS_DEBUG_LOG_LEVEL:
                            sys_print(
                                f"[authenticate] Found cached config for ID: {cached_id}",
                                is_debug=True,
                            )
                        ...
                        sys_print(
                            f"[authenticate] No cached config found for ID: {cached_id}"
                        )
            ...
            with tracer.start_as_current_span("retrieve_config") as retrieve_span:
                try:
                    if ENKRYPT_USE_REMOTE_MCP_CONFIG:
                        ...
                        if IS_DEBUG_LOG_LEVEL:
                            sys_print(
                                "[authenticate] Using local MCP config", is_debug=True
                            )
            ...
            with tracer.start_as_current_span("update_session") as session_span:
                ...
                if session_key not in SESSIONS:
                    SESSIONS[session_key] = {}
                SESSIONS[session_key].update(
                    {"authenticated": True, "gateway_config": gateway_config}
                )
            ...
            main_span.set_attribute(
                "auth_source", "remote" if ENKRYPT_USE_REMOTE_MCP_CONFIG else "local"
            )
            main_span.set_attribute("success", True)
...
async def enkrypt_list_all_servers(ctx: Context, discover_tools: bool = True):
    ...
    with tracer.start_as_current_span("enkrypt_list_all_servers") as main_span:
        ...
        gateway_config = get_local_mcp_config(
        ...
        sys_print("[list_available_servers] Request received")
        logger.info(
            "enkrypt_list_all_servers.started", extra=build_log_extra(ctx, custom_id)
        )
        try:
            ...
            if IS_DEBUG_LOG_LEVEL:
                logger.info(
                    "list_all_servers.returning_servers",
                    extra=build_log_extra(
                        ctx, custom_id, num_servers=len(servers_with_tools)
                    ),
                )
                sys_print(
                    f"[list_available_servers] Returning {len(servers_with_tools)} servers with tools",
                    is_debug=True,
                )

            if not discover_tools:
                main_span.set_attribute(
                    "total_servers_processed", len(servers_with_tools)
                )
                main_span.set_attribute("success", True)
                return {
                    "status": "success",
                ...
                main_span.set_attribute(
                    "servers_discovered", len(servers_needing_discovery)
                )
                main_span.set_attribute("success", True)
        ...
        try:
            ...
            with tracer.start_as_current_span("check_server_exists") as server_span:
                ...
                server_info = get_server_info_by_name(
                    SESSIONS[session_key]["gateway_config"], server_name
                )
            ...
            with tracer.start_as_current_span("get_latest_server_info") as info_span:
                ...
                info_span.set_attribute(
                    "gateway_id", SESSIONS[session_key]["gateway_config"]["id"]
                )
...
async def enkrypt_discover_all_tools(ctx: Context, server_name: str = None):
    ...
    with tracer.start_as_current_span("enkrypt_discover_all_tools") as main_span:
        ...
        try:
            # Authentication check
            if (
                session_key not in SESSIONS
                or not SESSIONS[session_key]["authenticated"]
            ):
            ...
            if not server_name:
                with tracer.start_as_current_span("discover_all_servers") as all_span:
                    ...
                    all_servers = await enkrypt_list_all_servers(
                        ctx, discover_tools=False
                    )
                    all_servers_with_tools = all_servers.get("available_servers", {})
                    servers_needing_discovery = all_servers.get(
                        "servers_needing_discovery", []
                    )

                    all_span.set_attribute(
                        "total_servers", len(servers_needing_discovery)
                    )
            ...
            with tracer.start_as_current_span("discover_tools") as discover_span:
                ...
                with tracer.start_as_current_span("forward_tool_call") as tool_span:
                    ...
                    result = await forward_tool_call(
                        server_name, None, None, SESSIONS[session_key]["gateway_config"]
                    )
                    end_time = time.time()
                    tool_call_duration.record(
                        end_time - start_time,
...
async def enkrypt_secure_call_tools(
    ...
    with tracer.start_as_current_span("enkrypt_secure_call_tools") as main_span:
        ...
        logger.info(
            "enkrypt_secure_call_tools.started",
            extra={
                "request_id": ctx.request_id,
                "custom_id": custom_id,
                "server_name": server_name,
        ...
        if num_tool_calls == 0:
            ...
            logger.info(
                "enkrypt_secure_call_tools.no_tools_provided",
                extra={
                    "request_id": ctx.request_id,
                    "custom_id": custom_id,
                    "server_name": server_name,
        ...
        try:
            ...
            with tracer.start_as_current_span("authenticate_gateway") as auth_span:
                ...
                enkrypt_project_name = gateway_config.get(
                    "project_name", "not_provided"
                )
                enkrypt_email = gateway_config.get("email", "not_provided")
                enkrypt_mcp_config_id = gateway_config.get(
                ...
                if (
                    ...
                    if result.get("status") != "success":
                        ...
                        logger.error(
                            "enkrypt_secure_call_tools.not_authenticated",
                            extra=build_log_extra(ctx, custom_id, server_name),
                        )
                        return {"status": "error", "error": "Not authenticated."}
                else:
            ...
            with tracer.start_as_current_span("validate_server_info") as server_span:
                ...
                if not server_info:
                    ...
                    logger.warning(
                        "enkrypt_secure_call_tools.server_not_available",
                        extra=build_log_extra(ctx, custom_id, server_name),
                    )
                    return {
                        "status": "error",
            ...
            if IS_DEBUG_LOG_LEVEL:
                ...
                sys_print(
                    f"Output Guardrails Policy: {output_guardrails_policy}",
                    is_debug=True,
                )
                logger.info(
                    "enkrypt_secure_call_tools.input_guardrails_policy",
                    extra=build_log_extra(
                        ctx,
                        custom_id,
                        server_name,
            ...
            sys_print(
                f"[secure_call_tools] Starting secure batch call for {num_tool_calls} tools for server: {server_name}"
            )
            logger.info(
                "enkrypt_secure_call_tools.starting_secure_batch_call",
                extra=build_log_extra(
                    ctx, custom_id, server_name, num_tool_calls=num_tool_calls
                ),
            )
            ...
            if IS_DEBUG_LOG_LEVEL:
                ...
                logger.info(
                    "enkrypt_secure_call_tools.using_command",
                    extra=build_log_extra(
            ...
            with tracer.start_as_current_span("tool_discovery") as discovery_span:
                ...
                discovery_span.set_attribute(
                    "has_cached_tools", bool(server_config_tools)
                )
                ...
                if IS_DEBUG_LOG_LEVEL:
                    ...
                    logger.info(
                        "enkrypt_secure_call_tools.server_config_tools_before_discovery",
                        extra=build_log_extra(
                            ctx,
                            custom_id,
                            server_name,
                ...
                if not server_config_tools:
                    server_config_tools = get_cached_tools(
                        cache_client, id, server_name
                    )
                    ...
                    if server_config_tools:
                        cache_hit_counter.add(1, attributes=build_log_extra(ctx))
                        logger.info(
                            "enkrypt_secure_call_tools.server_config_tools_after_get_cached_tools",
                            extra=build_log_extra(ctx, custom_id, server_name),
                        )
                    if IS_DEBUG_LOG_LEVEL:
                        logger.info(
                        ...
                        sys_print(
                            f"[secure_call_tools] Server config tools after get_cached_tools: {server_config_tools}",
                            is_debug=True,
                        )
                    if not server_config_tools:
                        cache_miss_counter.add(1, attributes=build_log_extra(ctx))
                        try:
                            ...
                            if IS_DEBUG_LOG_LEVEL:
                                sys_print(
                                    f"[enkrypt_secure_call_tools] Discovery result: {discovery_result}",
                                    is_debug=True,
                                )
                                logger.info(
                                    "enkrypt_secure_call_tools.discovery_result",
                                    extra=build_log_extra(
                                        ctx,
                                        custom_id,
                                        server_name,
                            ...
                            if discovery_result.get("status") != "success":
                                ...
                                logger.error(
                                    "enkrypt_secure_call_tools.discovery_failed",
                                    extra=build_log_extra(
                                        ctx,
                                        custom_id,
                                        server_name,
                                ...
                                return {
                                    "status": "error",
                                    "error": "Failed to discover tools for this server.",
                                }
                            ...
                            if IS_DEBUG_LOG_LEVEL:
                                ...
                                logger.info(
                                    "enkrypt_secure_call_tools.discovered_tools",
                                    extra=build_log_extra(
                                        ctx,
                                        custom_id,
                                        server_name,
                            ...
                            logger.error(
                                "enkrypt_secure_call_tools.exception",
                                extra=build_log_extra(
                                    ctx, custom_id, server_name, error=str(e)
                                ),
                            )
                            sys_print(
                                f"[enkrypt_secure_call_tools] Exception: {e}",
                                is_error=True,
                            )
                            traceback.print_exc(file=sys.stderr)
                            return {
                                "status": "error",
                                "error": f"Failed to discover tools: {e}",
                            }
                    else:
                        sys_print(
                ...
                if not server_config_tools:
                    ...
                    logger.error(
                        "enkrypt_secure_call_tools.no_tools_found",
                        extra=build_log_extra(ctx, custom_id, server_name),
                    )
                    sys_print(
                        f"[enkrypt_secure_call_tools] No tools found for {server_name} even after discovery",
                        is_error=True,
                    )
                    return {
                        "status": "error",
            ...
            if num_tool_calls == 0:
                ...
                if (
                    isinstance(server_config_tools, tuple)
                    and len(server_config_tools) == 2
                ):
                    server_config_tools = server_config_tools[
                        0
                    ]  # Extract the tools, ignoring expires_at
                return {
                    "status": "success",
            ...
            async with stdio_client(
                ...
                async with ClientSession(read, write) as session:
                    ...
                    logger.info(
                        "enkrypt_secure_call_tools.session_initialized",
                        extra=build_log_extra(ctx, custom_id, server_name),
                    )
                    ...
                    for i, tool_call in enumerate(tool_calls):
                        with tracer.start_as_current_span(
                            ...
                            try:
                                ...
                                logger.info(
                                    "enkrypt_secure_call_tools.processing_call",
                                    extra=build_log_extra(
                                        ctx,
                                        custom_id,
                                        server_name,
                                ...
                                with tracer.start_as_current_span(
                                    ...
                                    if server_config_tools:
                                        ...
                                        if (
                                            ...
                                            if IS_DEBUG_LOG_LEVEL:
                                                sys_print(
                                                    f"[secure_call_tools] server_config_tools is a tuple from cache: {server_config_tools}",
                                                    is_debug=True,
                                                )
                                                logger.info(
                                                    "enkrypt_secure_call_tools.server_config_tools_is_a_tuple_from_cache",
                                                    extra=build_log_extra(
                                                        ctx,
                                                        custom_id,
                                                        server_name,
                                        ...
                                        if hasattr(server_config_tools, "tools"):
                                            if IS_DEBUG_LOG_LEVEL:
                                                sys_print(
                                                    "[secure_call_tools] server_config_tools is a class with tools",
                                                    is_debug=True,
                                                )
                                                logger.info(
                                                    "enkrypt_secure_call_tools.server_config_tools_is_a_class_with_tools",
                                                    extra=build_log_extra(
                                                        ctx,
                                                        custom_id,
                                                        server_name,
                                            ...
                                            if isinstance(
                                                server_config_tools.tools, list
                                            ):
                                                # ListToolsResult format
                                                if IS_DEBUG_LOG_LEVEL:
                                                    sys_print(
                                                        f"[secure_call_tools] server_config_tools is ListToolsResult format: {server_config_tools}",
                                                        is_debug=True,
                                                    )
                                                    logger.info(
                                                        "enkrypt_secure_call_tools.server_config_tools_is_a_list_tools_result_format",
                                                        extra=build_log_extra(
                                                            ctx,
                                                            custom_id,
                                                            server_name,
                                                ...
                                                server_config_tools.tools, dict
                                            ):
                                                if IS_DEBUG_LOG_LEVEL:
                                                    sys_print(
                                                        "[secure_call_tools] server_config_tools.tools is in Dictionary format",
                                                        is_debug=True,
                                                    )
                                                    logger.info(
                                                        "enkrypt_secure_call_tools.server_config_tools_is_a_dict_with_tools",
                                                        extra=build_log_extra(
                                                            ctx,
                                                            custom_id,
                                                            server_name,
                                                ...
                                                if (
                                                    tool_name
                                                    in server_config_tools.tools
                                                ):
                                                    tool_found = True
                                        elif isinstance(server_config_tools, dict):
                                            if IS_DEBUG_LOG_LEVEL:
                                                sys_print(
                                                    "[secure_call_tools] server_config_tools is in Dictionary format",
                                                    is_debug=True,
                                                )
                                                logger.info(
                                                    "enkrypt_secure_call_tools.server_config_tools_is_a_dict",
                                                    extra=build_log_extra(
                                                        ctx,
                                                        custom_id,
                                                        server_name,
                                            ...
                                            if "tools" in server_config_tools:
                                                if IS_DEBUG_LOG_LEVEL:
                                                    sys_print(
                                                        "[secure_call_tools] server_config_tools is a dict and also has tools in Dictionary format",
                                                        is_debug=True,
                                                    )
                                                    logger.info(
                                                        "enkrypt_secure_call_tools.server_config_tools_is_a_dict_and_also_has_tools_in_dict_format",
                                                        extra=build_log_extra(
                                                            ctx,
                                                            custom_id,
                                                            server_name,
                                                ...
                                                if isinstance(
                                                    server_config_tools.get(
                                                        "tools", {}
                                                    ),
                                                    list,
                                                ):
                                                    for tool in server_config_tools.get(
                                                        "tools", []
                                                    ):
                                            ...
                                            # Dictionary format like {"echo": "Echo a message"}
                                                ...
                                                if IS_DEBUG_LOG_LEVEL:
                                                    sys_print(
                                                        f"[secure_call_tools] Tool '{tool_name}' not found in server_config_tools",
                                                        is_error=True,
                                                    )
                                                    logger.info(
                                                        "enkrypt_secure_call_tools.tool_not_found_in_server_config_tools",
                                                        extra=build_log_extra(
                                                            ctx,
                                                            custom_id,
                                                            server_name,
                                            ...
                                            logger.error(
                                                "enkrypt_secure_call_tools.unknown_tool_format",
                                                extra=build_log_extra(
                                                    ctx,
                                                    custom_id,
                                                    server_name,
                                    ...
                                    if not tool_found:
                                        ...
                                        logger.error(
                                            "enkrypt_secure_call_tools.tool_not_found_for_this_server",
                                            extra=build_log_extra(
                                                ctx,
                                                custom_id,
                                                server_name,
                                ...
                                if input_policy_enabled:
                                    with tracer.start_as_current_span(
                                        ...
                                        logger.info(
                                            "enkrypt_secure_call_tools.input_guardrails_enabled",
                                            extra=build_log_extra(
                                                ctx,
                                                custom_id,
                                                server_name,
                                        ...
                                        if ENKRYPT_ASYNC_INPUT_GUARDRAILS_ENABLED:
                                            input_span.set_attribute(
                                                "async_guardrails", True
                                            )
                                            guardrail_api_request_counter.add(
                                                1,
                                            ...
                                            end_time = time.time()
                                            guardrail_api_request_duration.record(
                                                end_time - start_time,
                                                attributes=build_log_extra(
                                                    ctx, custom_id
                                                ),
                                        ...
                                        # Check if guardrail response already includes PII redaction
                                        guardrail_has_pii = (
                                            guardrail_response_has_pii_redaction(
                                                input_guardrail_response
                                            )
                                        )
                                        ...
                                        if IS_DEBUG_LOG_LEVEL:
                                            sys_print(
                                                f"[secure_call_tools] Call {i}: Guardrail has PII: {guardrail_has_pii}",
                                                is_debug=True,
                                            )
                                            logger.info(
                                                "enkrypt_secure_call_tools.guardrail_has_pii",
                                                extra=build_log_extra(
                                                    ctx,
                                                    custom_id,
                                                    server_name,
                                                    tool_name=tool_name,
                                                    guardrail_has_pii=guardrail_has_pii,
                                                ),
                                            )

                                        # PII Redaction - only if not already included in guardrail response
                                        if pii_redaction and not guardrail_has_pii:
                                            start_time = time.time()
                                            sys_print(
                                                f"[secure_call_tools] Call {i}: PII redaction enabled for {tool_name} of server {server_name}"
                                            )
                                            logger.info(
                                                "enkrypt_secure_call_tools.pii_redaction_enabled",
                                                extra=build_log_extra(
                                                    ctx,
                                                    custom_id,
                                                    server_name,
                                            ...
                                            (
                                                anonymized_text,
                                                redaction_key,
                                            ) = anonymize_pii(input_json_string)
                                            pii_redactions_counter.add(
                                                1,
                                                attributes=build_log_extra(
                                                    ctx,
                                                    server_name=server_name,
                                            ...
                                            input_span.set_attribute(
                                                "pii_redaction_duration",
                                                end_time - start_time,
                                            )

                                            if IS_DEBUG_LOG_LEVEL:
                                                sys_print(
                                                    f"[secure_call_tools] Call {i}: Anonymized text: {anonymized_text}",
                                                    is_debug=True,
                                                )
                                                logger.info(
                                                    "enkrypt_secure_call_tools.anonymized_text",
                                                    extra=build_log_extra(
                                                        ctx,
                                                        custom_id,
                                                        server_name,
                                            ...
                                            # Using the anonymized text for tool call
                                            args = json.loads(anonymized_text)
                                        elif pii_redaction and guardrail_has_pii:
                                            # Extract anonymized text and redaction key from guardrail response
                                            try:
                                                pii_details = (
                                                    input_guardrail_response.get(
                                                        "details", {}
                                                    ).get("pii", {})
                                                )
                                                anonymized_text = pii_details.get(
                                                    "text", input_json_string
                                                )
                                                redaction_key = pii_details.get(
                                                    "key", ""
                                                )

                                                if IS_DEBUG_LOG_LEVEL:
                                                    sys_print(
                                                        f"[secure_call_tools] Call {i}: Using anonymized text from guardrail response: {anonymized_text}",
                                                        is_debug=True,
                                                    )
                                                    sys_print(
                                                        f"[secure_call_tools] Call {i}: Redaction key from guardrail response: {redaction_key}",
                                                        is_debug=True,
                                                    )
                                                    logger.info(
                                                        "enkrypt_secure_call_tools.using_guardrail_anonymized_text",
                                                        extra=build_log_extra(
                                                            ctx,
                                                            custom_id,
                                                            server_name,
                                                ...
                                                # Using the anonymized text from guardrail response for tool call
                                                args = json.loads(anonymized_text)

                                            except Exception as e:
                                                if IS_DEBUG_LOG_LEVEL:
                                                    sys_print(
                                                        f"[secure_call_tools] Call {i}: Error extracting anonymized text from guardrail response: {e}",
                                                        is_debug=True,
                                                    )
                                                    logger.info(
                                                        "enkrypt_secure_call_tools.error_extracting_guardrail_anonymized_text",
                                                        extra=build_log_extra(
                                                            ctx,
                                                            custom_id,
                                                            server_name,
                                                ...
                                                args = json.loads(input_json_string)

                                            if IS_DEBUG_LOG_LEVEL:
                                                sys_print(
                                                    f"[secure_call_tools] Call {i}: PII redaction already included in guardrail response for {tool_name} of server {server_name}",
                                                    is_debug=True,
                                                )
                                                logger.info(
                                                    "enkrypt_secure_call_tools.pii_redaction_already_included",
                                                    extra=build_log_extra(
                                                        ctx,
                                                        custom_id,
                                                        server_name,
                                            ...
                                            if IS_DEBUG_LOG_LEVEL:
                                                sys_print(
                                                    f"[secure_call_tools] Call {i}: PII redaction not enabled for {tool_name} of server {server_name}",
                                                    is_debug=True,
                                                )
                                                logger.info(
                                                    "enkrypt_secure_call_tools.pii_redaction_not_enabled",
                                                    extra=build_log_extra(
                                                        ctx,
                                                        custom_id,
                                                        server_name,
                                        ...
                                        input_span.set_attribute(
                                            "violation_types",
                                            str(input_violation_types),
                                        )

                                        sys_print(
                                            f"input_violations: {input_violations_detected}, {input_violation_types}"
                                        )
                                        logger.info(
                                            "enkrypt_secure_call_tools.input_violations",
                                            extra=build_log_extra(
                                                ctx,
                                                custom_id,
                                                server_name,
                                        ...
                                        if input_violations_detected:
                                            ...
                                            input_span.set_attribute(
                                                "error",
                                                f"Input violations: {input_violation_types}",
                                            )
                                            for violation_type in input_violation_types:
                                                guardrail_violation_counter.add(
                                                    1,
                                                    attributes=build_log_extra(
                                                        ctx,
                                                ...
                                                input_guardrail_violation_counter.add(
                                                    1,
                                                    attributes=build_log_extra(
                                                        ctx,
                                                        server_name=server_name,
                                            ...
                                            logger.info(
                                                "enkrypt_secure_call_tools.blocked_due_to_input_violations",
                                                extra=build_log_extra(
                                                    ctx,
                                                    custom_id,
                                                    server_name,
                                        ...
                                        if ENKRYPT_ASYNC_INPUT_GUARDRAILS_ENABLED:
                                            if IS_DEBUG_LOG_LEVEL:
                                                sys_print(
                                                    f"[secure_call_tools] Call {i}: Waiting for tool call to complete in async mode",
                                                    is_debug=True,
                                                )
                                            logger.info(
                                                "enkrypt_secure_call_tools.waiting_for_tool_call_to_complete_in_async_mode",
                                                extra=build_log_extra(
                                                    ctx,
                                                    custom_id,
                                                    server_name,
                                    ...
                                    logger.info(
                                        "enkrypt_secure_call_tools.input_guardrails_not_enabled",
                                        extra=build_log_extra(
                                            ctx,
                                            custom_id,
                                            server_name,
                                ...
                                if IS_DEBUG_LOG_LEVEL:
                                    sys_print(
                                        f"[secure_call_tools] Call {i}: Success: {server_name}.{tool_name}",
                                        is_debug=True,
                                    )
                                    sys_print(
                                        f"[secure_call_tools] Call {i}: type of result: {type(result)}",
                                        is_debug=True,
                                    )
                                    sys_print(
                                        f"[secure_call_tools] Call {i}: Tool call result: {result}",
                                        is_debug=True,
                                    )
                                    logger.info(
                                        "enkrypt_secure_call_tools.tool_call_result",
                                        extra=build_log_extra(
                                            ctx,
                                            custom_id,
                                            server_name,
                                ...
                                if (
                                    ...
                                    if result_type == "text":
                                        ...
                                        sys_print(
                                            f"[secure_call_tools] Call {i}: Tool executed and is text, checking output guardrails"
                                        )
                                        logger.info(
                                            "enkrypt_secure_call_tools.tool_executed_and_is_text_checking_output_guardrails",
                                            extra=build_log_extra(
                                                ctx,
                                                custom_id,
                                                server_name,
                                        ...
                                        sys_print(
                                            f"[secure_call_tools] Call {i}: Tool result is not text, skipping output guardrails"
                                        )
                                        logger.info(
                                            "enkrypt_secure_call_tools.tool_result_is_not_text_skipping_output_guardrails",
                                            extra=build_log_extra(
                                                ctx,
                                                custom_id,
                                                server_name,
                                ...
                                if text_result:
                                    ...
                                    with tracer.start_as_current_span(
                                        ...
                                        output_span.set_attribute(
                                            "relevancy_enabled", relevancy
                                        )
                                        output_span.set_attribute(
                                            "adherence_enabled", adherence
                                        )
                                        output_span.set_attribute(
                                        ...
                                        if not ENKRYPT_ASYNC_OUTPUT_GUARDRAILS_ENABLED:
                                            sys_print(
                                                f"[secure_call_tools] Call {i}: Starting sync output guardrails for {tool_name} of server {server_name}"
                                            )
                                            logger.info(
                                                "enkrypt_secure_call_tools.starting_sync_output_guardrails",
                                                extra=build_log_extra(
                                                    ctx,
                                                    custom_id,
                                                    server_name,
                                            ...
                                            if output_policy_enabled:
                                                sys_print(
                                                    f"[secure_call_tools] Call {i}: Output guardrails enabled for {tool_name} of server {server_name}"
                                                )
                                                logger.info(
                                                    "enkrypt_secure_call_tools.output_guardrails_enabled",
                                                    extra=build_log_extra(
                                                        ctx,
                                                        custom_id,
                                                        server_name,
                                                ...
                                                sys_print(
                                                    f"output_violation_types: {output_violation_types}"
                                                )
                                                logger.info(
                                                    "enkrypt_secure_call_tools.output_guardrails_processing",
                                                    extra=build_log_extra(
                                                        ctx,
                                                        custom_id,
                                                        server_name,
                                                ...
                                                if output_violations_detected:
                                                    ...
                                                    output_span.set_attribute(
                                                        "error",
                                                        f"Output violations: {output_violation_types}",
                                                    )
                                                    for (
                                                        violation_type
                                                    ) in output_violation_types:
                                                        guardrail_violation_counter.add(
                                                            1,
                                                            attributes=build_log_extra(
                                                                ctx,
                                                        ...
                                                        output_guardrail_violation_counter.add(
                                                            1,
                                                            attributes=build_log_extra(
                                                                ctx,
                                                                server_name=server_name,
                                                    ...
                                                    logger.info(
                                                        "enkrypt_secure_call_tools.blocked_due_to_output_violations",
                                                        extra=build_log_extra(
                                                            ctx,
                                                            custom_id,
                                                            server_name,
                                                    ...
                                                    sys_print(
                                                        f"[secure_call_tools] Call {i}: No output violations detected for {tool_name} of server {server_name}"
                                                    )
                                                    logger.info(
                                                        "enkrypt_secure_call_tools.no_output_violations_detected",
                                                        extra=build_log_extra(
                                                            ctx,
                                                            custom_id,
                                                            server_name,
                                            ...
                                            if relevancy:
                                                sys_print(
                                                    f"[secure_call_tools] Call {i}: Checking relevancy for {tool_name} of server {server_name}"
                                                )
                                                logger.info(
                                                    "enkrypt_secure_call_tools.checking_relevancy",
                                                    extra=build_log_extra(
                                                        ctx,
                                                        custom_id,
                                                        server_name,
                                                ...
                                                output_span.set_attribute(
                                                    "relevancy_check_duration",
                                                    end_time - start_time,
                                                )

                                                if IS_DEBUG_LOG_LEVEL:
                                                    sys_print(
                                                        f"relevancy response: {output_relevancy_response}",
                                                        is_debug=True,
                                                    )
                                                    logger.info(
                                                        "enkrypt_secure_call_tools.relevancy_response",
                                                        extra=build_log_extra(
                                                            ctx,
                                                            custom_id,
                                                            server_name,
                                                ...
                                                if (
                                                    "relevancy" in output_blocks
                                                    and output_relevancy_response.get(
                                                        "summary", {}
                                                    ).get("relevancy_score")
                                                    > RELEVANCY_THRESHOLD
                                                ):
                                                    output_span.set_attribute(
                                                        "relevancy_violation", True
                                                    )
                                                    relevancy_violation_counter.add(
                                                        1,
                                                        attributes=build_log_extra(
                                                            ctx,
                                                            custom_id,
                                                            server_name,
                                                            tool_name=tool_name,
                                                            violation_type="relevancy",
                                                        ),
                                                    )
                                                    logger.info(
                                                        "enkrypt_secure_call_tools.blocked_due_to_relevancy_violations",
                                                        extra=build_log_extra(
                                                            ctx,
                                                            custom_id,
                                                            server_name,
                                                    ...
                                                    output_span.set_attribute(
                                                        "relevancy_violation", False
                                                    )
                                                    sys_print(
                                                        f"[secure_call_tools] Call {i}: No relevancy violations detected or relevancy is not in output_blocks for {tool_name} of server {server_name}"
                                                    )
                                                    logger.info(
                                                        "enkrypt_secure_call_tools.no_relevancy_violations_detected",
                                                        extra=build_log_extra(
                                                            ctx,
                                                            custom_id,
                                                            server_name,
                                            ...
                                            if adherence:
                                                sys_print(
                                                    f"[secure_call_tools] Call {i}: Checking adherence for {tool_name} of server {server_name}"
                                                )
                                                logger.info(
                                                    "enkrypt_secure_call_tools.checking_adherence",
                                                    extra=build_log_extra(
                                                        ctx,
                                                        custom_id,
                                                        server_name,
                                                ...
                                                output_span.set_attribute(
                                                    "adherence_check_duration",
                                                    end_time - start_time,
                                                )

                                                if IS_DEBUG_LOG_LEVEL:
                                                    sys_print(
                                                        f"adherence response: {output_adherence_response}",
                                                        is_debug=True,
                                                    )
                                                    logger.info(
                                                        "enkrypt_secure_call_tools.adherence_response",
                                                        extra=build_log_extra(
                                                            ctx,
                                                            custom_id,
                                                            server_name,
                                                ...
                                                if (
                                                    "adherence" in output_blocks
                                                    and output_adherence_response.get(
                                                        "summary", {}
                                                    ).get("adherence_score")
                                                    > ADHERENCE_THRESHOLD
                                                ):
                                                    output_span.set_attribute(
                                                        "adherence_violation", True
                                                    )
                                                    adherence_violation_counter.add(
                                                        1,
                                                        attributes=build_log_extra(
                                                            ctx,
                                                            violation_type="adherence",
                                                            server_name=server_name,
                                                            tool=tool_name,
                                                        ),
                                                    )
                                                    logger.info(
                                                        "enkrypt_secure_call_tools.blocked_due_to_adherence_violations",
                                                        extra=build_log_extra(
                                                            ctx,
                                                            custom_id,
                                                            server_name,
                                                    ...
                                                    output_span.set_attribute(
                                                        "adherence_violation", False
                                                    )
                                                    sys_print(
                                                        f"[secure_call_tools] Call {i}: No adherence violations detected or adherence is not in output_blocks for {tool_name} of server {server_name}"
                                                    )
                                                    logger.info(
                                                        "enkrypt_secure_call_tools.no_adherence_violations_detected",
                                                        extra=build_log_extra(
                                                            ctx,
                                                            custom_id,
                                                            server_name,
                                            ...
                                            if hallucination:
                                                sys_print(
                                                    f"[secure_call_tools] Call {i}: Checking hallucination for {tool_name} of server {server_name}"
                                                )
                                                logger.info(
                                                    "enkrypt_secure_call_tools.checking_hallucination",
                                                    extra=build_log_extra(
                                                        ctx,
                                                        custom_id,
                                                        server_name,
                                                ...
                                                output_span.set_attribute(
                                                    "hallucination_check_duration",
                                                    end_time - start_time,
                                                )

                                                if IS_DEBUG_LOG_LEVEL:
                                                    sys_print(
                                                        f"hallucination response: {output_hallucination_response}",
                                                        is_debug=True,
                                                    )
                                                    logger.info(
                                                        "enkrypt_secure_call_tools.hallucination_response",
                                                        extra=build_log_extra(
                                                            ctx,
                                                            custom_id,
                                                            server_name,
                                                ...
                                                if (
                                                    "hallucination" in output_blocks
                                                    and output_hallucination_response.get(
                                                        "summary", {}
                                                    ).get("is_hallucination")
                                                    > 0
                                                ):
                                                    output_span.set_attribute(
                                                        "hallucination_violation", True
                                                    )
                                                    hallucination_violation_counter.add(
                                                        1,
                                                        attributes=build_log_extra(
                                                            ctx,
                                                    ...
                                                    logger.info(
                                                        "enkrypt_secure_call_tools.blocked_due_to_hallucination_violations",
                                                        extra=build_log_extra(
                                                            ctx,
                                                            custom_id,
                                                            server_name,
                                                    ...
                                                    sys_print(
                                                        f"[secure_call_tools] Call {i}: No hallucination violations detected or hallucination is not in output_blocks for {tool_name} of server {server_name}"
                                                    )
                                                    logger.info(
                                                        "enkrypt_secure_call_tools.no_hallucination_violations_detected",
                                                        extra=build_log_extra(
                                                            ctx,
                                                            custom_id,
                                                            server_name,
                                            ...
                                            if pii_redaction and redaction_key:
                                                start_time = time.time()
                                                sys_print(
                                                    f"[secure_call_tools] Call {i}: De-anonymizing PII for {tool_name} of server {server_name} with redaction key: {redaction_key}"
                                                )
                                                logger.info(
                                                    "enkrypt_secure_call_tools.deanonymizing_pii",
                                                    extra=build_log_extra(
                                                        ctx,
                                                        custom_id,
                                                        server_name,
                                                ...
                                                deanonymized_text = deanonymize_pii(
                                                    text_result, redaction_key
                                                )
                                                end_time = time.time()
                                                guardrail_api_request_duration.record(
                                                    end_time - start_time,
                                                ...
                                                output_span.set_attribute(
                                                    "pii_deanonymization_duration",
                                                    end_time - start_time,
                                                )

                                                if IS_DEBUG_LOG_LEVEL:
                                                    sys_print(
                                                        f"[secure_call_tools] Call {i}: De-anonymized text for {tool_name} of server {server_name}: {deanonymized_text}",
                                                        is_debug=True,
                                                    )
                                                    logger.info(
                                                        "enkrypt_secure_call_tools.deanonymized_text",
                                                        extra=build_log_extra(
                                                            ctx,
                                                            custom_id,
                                                            server_name,
                                                ...
                                                sys_print(
                                                    f"[secure_call_tools] Call {i}: PII redaction not enabled or redaction key {redaction_key} not found for {tool_name} of server {server_name}"
                                                )
                                                logger.info(
                                                    "enkrypt_secure_call_tools.pii_redaction_not_enabled",
                                                    extra=build_log_extra(
                                    ...
                                    if ENKRYPT_ASYNC_OUTPUT_GUARDRAILS_ENABLED:
                                        sys_print(
                                            f"[secure_call_tools] Call {i}: Starting async output guardrails for {tool_name} of server {server_name}"
                                        )
                                        logger.info(
                                            "enkrypt_secure_call_tools.starting_async_output_guardrails",
                                            extra=build_log_extra(
                                                ctx,
                                                custom_id,
                                                server_name,
                                        ...
                                        # Relevancy check task
                                        if relevancy:
                                            sys_print(
                                                f"[secure_call_tools] Call {i}: Starting async relevancy check for {tool_name} of server {server_name}"
                                            )
                                            logger.info(
                                                "enkrypt_secure_call_tools.starting_async_relevancy_check",
                                                extra=build_log_extra(
                                                    ctx,
                                                    custom_id,
                                                    server_name,
                                        ...
                                        # Adherence check task
                                        if adherence:
                                            sys_print(
                                                f"[secure_call_tools] Call {i}: Starting async adherence check for {tool_name} of server {server_name}"
                                            )
                                            logger.info(
                                                "enkrypt_secure_call_tools.starting_async_adherence_check",
                                                extra=build_log_extra(
                                                    ctx,
                                                    custom_id,
                                                    server_name,
                                        ...
                                        # Hallucination check task
                                        if hallucination:
                                            sys_print(
                                                f"[secure_call_tools] Call {i}: Starting async hallucination check for {tool_name} of server {server_name}"
                                            )
                                            logger.info(
                                                "enkrypt_secure_call_tools.starting_async_hallucination_check",
                                                extra=build_log_extra(
                                                    ctx,
                                                    custom_id,
                                                    server_name,
                                        ...
                                        # Process output guardrail result
                                        if "output_guardrail" in tasks:
                                            ...
                                            logger.info(
                                                "enkrypt_secure_call_tools.async_output_guardrails_processing",
                                                extra=build_log_extra(
                                                    ctx,
                                                    custom_id,
                                                    server_name,
                                            ...
                                            if output_violations_detected:
                                                ...
                                                # Cancel remaining tasks since we're blocking
                                                for task_name, task in tasks.items():
                                                    if (
                                                        task_name != "output_guardrail"
                                                        and not task.done()
                                                    ):
                                                        task.cancel()
                                        ...
                                        # Process relevancy result
                                        if "relevancy" in tasks:
                                            start_time = time.time()
                                            output_relevancy_response = await tasks[
                                                "relevancy"
                                            ]
                                            end_time = time.time()
                                            guardrail_api_request_duration.record(
                                            ...
                                            if IS_DEBUG_LOG_LEVEL:
                                                sys_print(
                                                    f"async relevancy response: {output_relevancy_response}",
                                                    is_debug=True,
                                                )
                                                logger.info(
                                                    "enkrypt_secure_call_tools.async_relevancy_response",
                                                    extra=build_log_extra(
                                                        ctx,
                                                        custom_id,
                                                        server_name,
                                            ...
                                            if (
                                                ...
                                                logger.info(
                                                    "enkrypt_secure_call_tools.blocked_due_to_async_relevancy_violations",
                                                    extra=build_log_extra(
                                                        ctx,
                                                        custom_id,
                                                        server_name,
                                                ...
                                                sys_print(
                                                    f"[secure_call_tools] Call {i}: No relevancy violations detected in async mode for {tool_name} of server {server_name}"
                                                )
                                                logger.info(
                                                    "enkrypt_secure_call_tools.no_async_relevancy_violations_detected",
                                                    extra=build_log_extra(
                                                        ctx,
                                                        custom_id,
                                                        server_name,
                                        ...
                                        # Process adherence result
                                        if "adherence" in tasks:
                                            start_time = time.time()
                                            output_adherence_response = await tasks[
                                                "adherence"
                                            ]
                                            end_time = time.time()
                                            guardrail_api_request_duration.record(
                                            ...
                                            if IS_DEBUG_LOG_LEVEL:
                                                sys_print(
                                                    f"async adherence response: {output_adherence_response}",
                                                    is_debug=True,
                                                )
                                                logger.info(
                                                    "enkrypt_secure_call_tools.async_adherence_response",
                                                    extra=build_log_extra(
                                                        ctx,
                                                        custom_id,
                                                        server_name,
                                            ...
                                            if (
                                                ...
                                                logger.info(
                                                    "enkrypt_secure_call_tools.blocked_due_to_async_adherence_violations",
                                                    extra=build_log_extra(
                                                        ctx,
                                                        custom_id,
                                                        server_name,
                                                ...
                                                sys_print(
                                                    f"[secure_call_tools] Call {i}: No adherence violations detected in async mode for {tool_name} of server {server_name}"
                                                )
                                                logger.info(
                                                    "enkrypt_secure_call_tools.no_async_adherence_violations_detected",
                                                    extra=build_log_extra(
                                                        ctx,
                                                        custom_id,
                                                        server_name,
                                        ...
                                        if "hallucination" in tasks:
                                            start_time = time.time()
                                            output_hallucination_response = await tasks[
                                                "hallucination"
                                            ]
                                            end_time = time.time()
                                            guardrail_api_request_duration.record(
                                            ...
                                            if IS_DEBUG_LOG_LEVEL:
                                                sys_print(
                                                    f"async hallucination response: {output_hallucination_response}",
                                                    is_debug=True,
                                                )
                                                logger.info(
                                                    "enkrypt_secure_call_tools.async_hallucination_response",
                                                    extra=build_log_extra(
                                                        ctx,
                                                        custom_id,
                                                        server_name,
                                            ...
                                            if (
                                                ...
                                                logger.info(
                                                    "enkrypt_secure_call_tools.blocked_due_to_async_hallucination_violations",
                                                    extra=build_log_extra(
                                                        ctx,
                                                        custom_id,
                                                        server_name,
                                                ...
                                                sys_print(
                                                    f"[secure_call_tools] Call {i}: No hallucination violations detected in async mode for {tool_name} of server {server_name}"
                                                )
                                                logger.info(
                                                    "enkrypt_secure_call_tools.no_async_hallucination_violations_detected",
                                                    extra=build_log_extra(
                                                        ctx,
                                                        custom_id,
                                                        server_name,
                                ...
                                logger.info(
                                    "enkrypt_secure_call_tools.completed_successfully",
                                    extra=build_log_extra(
                                        ctx, custom_id, server_name, tool_name=tool_name
                                    ),
                                )
                                # Successful result
                                results.append(
                                ...
                                tool_call_success_counter.add(
                                    1,
                                    attributes=build_log_extra(
                                        ctx, server_name=server_name, tool=tool_name
                                    ),
                                )
                                ...
                                traceback.print_exc(file=sys.stderr)
                                logger.error(
                                    "enkrypt_secure_call_tools.error_in_tool_call",
                                    extra=build_log_extra(
                                        ctx,
                                        custom_id,
                                        server_name,
            ...
            blocked_calls = len(
                [r for r in results if r["status"].startswith("blocked")]
            )
            failed_calls = len([r for r in results if r["status"] == "error"])

            tool_call_failure_counter.add(
                failed_calls, attributes=build_log_extra(ctx, server_name=server_name)
            )
            tool_call_error_counter.add(
                failed_calls, attributes=build_log_extra(ctx, server_name=server_name)
            )
            tool_call_blocked_counter.add(
                blocked_calls, attributes=build_log_extra(ctx, server_name=server_name)
            )
            ...
            logger.info(
                "enkrypt_secure_call_tools.batch_execution_completed",
                extra=build_log_extra(
                    ctx,
                    custom_id,
                    server_name,
            ...
            traceback.print_exc(file=sys.stderr)
            logger.error(
                "enkrypt_secure_call_tools.critical_error_during_batch_execution",
    ...
    with tracer.start_as_current_span("enkrypt_get_cache_status") as main_span:
        try:
            ...
            with tracer.start_as_current_span("authenticate_gateway") as auth_span:
                ...
                if (
                    ...
                    if result.get("status") != "success":
                        ...
                        logger.error(
                            "enkrypt_get_cache_status.not_authenticated",
                            extra=build_log_extra(
            ...
            with tracer.start_as_current_span("get_cache_statistics") as stats_span:
                ...
                logger.info(
                    "enkrypt_get_cache_status.getting_cache_statistics",
            ...
            with tracer.start_as_current_span(
                ...
                logger.info(
                    "enkrypt_get_cache_status.getting_gateway_config",
                ...
                if cached_result:
                    ...
                    if isinstance(cached_result, tuple) and len(cached_result) == 2:
                        ...
                        if ENKRYPT_MCP_USE_EXTERNAL_CACHE and cache_client:
                            ...
                            ttl_seconds, expires_at = get_redis_ttl(
                                cache_client, config_key
                            )
                            if IS_DEBUG_LOG_LEVEL:
                                sys_print(
                    ...
                    if IS_DEBUG_LOG_LEVEL:
                        ...
                        logger.info(
                            "enkrypt_get_cache_status.cached_gateway_config",
                            extra=build_log_extra(
                    ...
                    logger.info(
                        "enkrypt_get_cache_status.no_cached_gateway_config_found",
                        extra=build_log_extra(ctx, custom_id, id=id),
                    )
            ...
            with tracer.start_as_current_span(
                "check_server_tools_cache"
            ) as servers_span:
                sys_print(
                    "[get_cache_status] Getting server cache status", is_debug=True
                )
                logger.info(
                    "enkrypt_get_cache_status.getting_server_cache_status",
                    extra=build_log_extra(ctx, custom_id, id=id),
                )
                ...
                if IS_DEBUG_LOG_LEVEL:
                    ...
                    logger.info(
                        "enkrypt_get_cache_status.mcp_configs",
                ...
                if IS_DEBUG_LOG_LEVEL:
                    ...
                    logger.info(
                        "enkrypt_get_cache_status.local_gateway_config",
                        extra=build_log_extra(
                ...
                for server_info in mcp_config:
                    ...
                    with tracer.start_as_current_span(
                        f"check_server_cache_{server_name}"
                    ) as server_span:
                        server_span.set_attribute("server_name", server_name)
                        ...
                        if IS_DEBUG_LOG_LEVEL:
                            ...
                            logger.info(
                                "enkrypt_get_cache_status.getting_tool_cache",
                                extra=build_log_extra(
                                    ctx, custom_id, id=id, server_name=server_name
                                ),
                            )
                        ...
                        if IS_DEBUG_LOG_LEVEL:
                            sys_print(
                                f"[get_cache_status] Cached result: {cached_result}",
                                is_debug=True,
                            )
                            logger.info(
                                "enkrypt_get_cache_status.cached_result",
                                extra=build_log_extra(
                                    ctx,
                                    custom_id,
                                    id=id,
                        ...
                        if cached_result:
                            ...
                            if (
                                ...
                                if ENKRYPT_MCP_USE_EXTERNAL_CACHE and cache_client:
                                    ...
                                    tools_key = get_server_hashed_key(id, server_name)
                                    ttl_seconds, expires_at = get_redis_ttl(
                                        cache_client, tools_key
                                    )
                                    if IS_DEBUG_LOG_LEVEL:
                                        sys_print(
                            ...
                            if IS_DEBUG_LOG_LEVEL:
                                ...
                                logger.info(
                                    "enkrypt_get_cache_status.tools_found",
                                    extra=build_log_extra(
                                        ctx,
                                        custom_id,
                                        id=id,
                                ...
                                if hasattr(tools, "tools") and isinstance(
                                    tools.tools, list
                                ):
                                ...
                                # Handle flat dictionary format
                                    ...
                                    tool_count = None
                                    logger.error(
                                        "enkrypt_get_cache_status.unknown_tool_format",
                                        extra=build_log_extra(
                                            ctx,
                                            custom_id,
                                            id=id,
                            ...
                            if local_gateway_config:
                                ...
                                if local_server_info and "tools" in local_server_info:
                                    if IS_DEBUG_LOG_LEVEL:
                                        ...
                                        logger.info(
                                            "enkrypt_get_cache_status.server_tools_defined_in_local_gateway_config",
                                            extra=build_log_extra(
                                                ctx,
                                                custom_id,
                                                id=id,
                                ...
                                if IS_DEBUG_LOG_LEVEL:
                                    sys_print(
                                        f"[get_cache_status] No tools found for server that needs discovery: {server_name}",
                                        is_debug=True,
                                    )
                                    logger.info(
                                        "enkrypt_get_cache_status.no_tools_found_for_server_that_needs_discovery",
                                        extra=build_log_extra(
                                            ctx,
                                            custom_id,
                                            id=id,
                ...
                servers_span.set_attribute(
                    "servers_need_discovery", servers_need_discovery
                )
            ...
            logger.error(
                "enkrypt_get_cache_status.critical_error",
                extra=build_log_extra(ctx, custom_id, error=str(e)),
            )
            raise
    ...
    with tracer.start_as_current_span("enkrypt_clear_cache") as main_span:
        try:
            ...
            with tracer.start_as_current_span("authenticate_gateway") as auth_span:
                ...
                if not gateway_config:
                    sys_print(
                        f"[enkrypt_clear_cache] No local MCP config found for gateway_key={mask_key(enkrypt_gateway_key)}, project_id={enkrypt_project_id}, user_id={enkrypt_user_id}",
                        is_error=True,
                    )
                    return {
                        "status": "error",
                ...
                if (
                    ...
                    if result.get("status") != "success":
                        ...
                        logger.error(
                            "enkrypt_clear_cache.not_authenticated",
                            extra=build_log_extra(
                                ctx, custom_id, error="Not authenticated."
                            ),
                        )
            ...
            with tracer.start_as_current_span("determine_cache_type") as type_span:
                if not cache_type:
                    ...
                    if IS_DEBUG_LOG_LEVEL:
                        ...
                        logger.info(
                            "enkrypt_clear_cache.no_cache_type_provided",
                            extra=build_log_extra(
                                ctx,
                                custom_id,
                                id=id,
            ...
            if cache_type == "all":
                with tracer.start_as_current_span("clear_all_caches") as all_span:
                    try:
                        ...
                        logger.info(
                            "enkrypt_clear_cache.clearing_all_caches",
                            extra=build_log_extra(
                                ctx,
                                custom_id,
                                id=id,
                        ...
                        cleared_gateway = clear_gateway_config_cache(
                            cache_client, id, enkrypt_gateway_key
                        )

                        all_span.set_attribute("cleared_servers_count", cleared_servers)
                        all_span.set_attribute(
                            "gateway_config_cleared", cleared_gateway
                        )
                        ...
                        if ENKRYPT_USE_REMOTE_MCP_CONFIG:
                            with tracer.start_as_current_span(
                                ...
                                if IS_DEBUG_LOG_LEVEL:
                                    ...
                                    logger.info(
                                        "enkrypt_clear_cache.refreshing_remote_mcp_config",
                                        extra=build_log_extra(
                                            ctx,
                                            custom_id,
                                            id=id,
                                ...
                                refresh_span.set_attribute(
                                    "status_code", refresh_response.status_code
                                )
                                refresh_span.set_attribute(
                                    "success", refresh_response.ok
                                )
                                ...
                                if IS_DEBUG_LOG_LEVEL:
                                    ...
                                    logger.info(
                                        "enkrypt_clear_cache.refresh_response",
                                        extra=build_log_extra(
                                            ctx,
                                            custom_id,
                                            id=id,
            ...
            if (
                cache_type == "gateway_config"
                or cache_type == "gateway"
                or cache_type == "gateway_cache"
                or cache_type == "gateway_config_cache"
            ):
                with tracer.start_as_current_span(
                    ...
                    try:
                        ...
                        logger.info(
                            "enkrypt_clear_cache.clearing_gateway_config_cache",
                            extra=build_log_extra(
                                ctx,
                                custom_id,
                                id=id,
                        ...
                        cleared = clear_gateway_config_cache(
                            cache_client, id, enkrypt_gateway_key
                        )
                        ...
                        if cleared:
                            logger.info(
                                "enkrypt_clear_cache.gateway_config_cache_cleared",
                                extra=build_log_extra(
                                    ctx,
                                    custom_id,
                                    id=id,
                            ...
                            logger.info(
                                "enkrypt_clear_cache.no_config_cache_found",
                                extra=build_log_extra(
                                    ctx,
                                    custom_id,
                                    id=id,
            ...
            with tracer.start_as_current_span("clear_server_cache") as server_span:
                try:
                    ...
                    server_span.set_attribute(
                        "clear_specific_server", bool(server_name)
                    )

                    sys_print("[clear_cache] Clearing server config cache")
                    logger.info(
                        "enkrypt_clear_cache.clearing_server_config_cache",
                        extra=build_log_extra(
                            ctx,
                            custom_id,
                            id=id,
                    ...
                    if server_name:
                        if IS_DEBUG_LOG_LEVEL:
                            ...
                            logger.info(
                                "enkrypt_clear_cache.clearing_tool_cache_for_server",
                                extra=build_log_extra(
                                    ctx,
                                    custom_id,
                                    id=id,
                        ...
                        if cleared:
                            logger.info(
                                "enkrypt_clear_cache.tool_cache_cleared",
                                extra=build_log_extra(
                                    ctx,
                                    custom_id,
                                    id=id,
                    ...
                    # Clear all server caches (tool cache)
                        ...
                        logger.info(
                            "enkrypt_clear_cache.clearing_all_server_caches",
                            extra=build_log_extra(
                                ctx,
                                custom_id,
                                id=id,
            ...
            logger.error(
                "enkrypt_clear_cache.critical_error",
                extra=build_log_extra(ctx, custom_id, error=str(e)),
            )
            raise
