...
async def enkrypt_list_all_servers(ctx: Context, discover_tools: bool = True):
    ...
    with tracer.start_as_current_span("enkrypt_list_all_servers") as span:
        ...
        try:
            with tracer.start_span("enkrypt_authenticate") as auth_span:
...
async def enkrypt_get_server_info(ctx: Context, server_name: str):
    ...
    enkrypt_gateway_key = get_gateway_key(ctx)
    with tracer.start_as_current_span("enkrypt_get_server_info") as span:
        ...
        try:
            ...
            with tracer.start_span("get_latest_server_info") as info_span:
...
async def enkrypt_discover_all_tools(ctx: Context, server_name: str = None):
    ...
    with tracer.start_as_current_span("enkrypt_discover_all_tools") as span:
        span.set_attribute("server_name", server_name)
        span.set_attribute("custom_id", custom_id)
        span.set_attribute("job", "enkrypt")
        span.set_attribute("env", "dev")
        ...
        if not server_name:
             with tracer.start_span("discover_all_servers") as all_span:    
        ...
        with tracer.start_span("get_server_info") as info_span:
            server_info = get_server_info_by_name(SESSIONS[enkrypt_gateway_key]["gateway_config"], server_name)
            if not server_info:
                if IS_DEBUG_LOG_LEVEL:
        ...
        try:
            with tracer.start_span("discover_tools") as discover_span:
                discover_span.set_attribute("server_name", server_name)
                cached_tools = get_cached_tools(cache_client, id, server_name)
                with tracer.start_span("check_cache") as cache_span:
                    cached_tools = get_cached_tools(cache_client, id, server_name)
                    if cached_tools:
                        cache_hit_counter.add(1)
                    ...
                    with tracer.start_span("forward_tool_call") as tool_span:
                        tool_call_counter.add(1)
                        start_time = time.time()
                        ...
                        if tools:
                            ...
                            with tracer.start_span("cache_tools") as cache_write_span:
            ...
            if not server_config_tools:
                ...
                try:
                    ...
                    if discovery_result.get("status") == "success":
                        ...
                        # Add counter for newly discovered server
